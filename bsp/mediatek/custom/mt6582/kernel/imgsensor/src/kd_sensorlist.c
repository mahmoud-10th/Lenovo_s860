...
/*Lenovo-sw chenglong1 add for sensor reg debug begin*/
static CAMERA_DUAL_CAMERA_SENSOR_ENUM g_cur_active_sensor = DUAL_CAMERA_MAIN_SENSOR;
/*Lenovo-sw add end*/
/*=============================================================================
...
int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId)
{
    ...
    int retry = 0;
	    ...
	    buf = (u8 *)dma_alloc_coherent(NULL, bytes, &phyAddr, GFP_KERNEL);
...
#if 0
		g_pstI2Cclient->addr = (g_pstI2Cclient->addr & (~0xff)) |i2cId;
	    g_pstI2Cclient->addr = ( (((g_pstI2Cclient->addr|i2cId) >> 1) &  I2C_MASK_FLAG) | I2C_DMA_FLAG );
#else
	    g_pstI2Cclient->addr =  i2cId>>1;//(g_pstI2Cclient->addr >> 1);
        g_pstI2Cclient->ext_flag= I2C_DMA_FLAG;// |I2C_ENEXT_FLAG;
#endif
		spin_unlock(&kdsensor_drv_lock);
	    ...
	    do {
	        ...
	        retry --;
	        if (ret != bytes) {
	            printk(KERN_ERR "Error sent I2C ret = %d\n", ret);
	        }
...
int kdSetDriver(unsigned int* pDrvIndex)
{
    ...
    /*Lenovo-sw chenglong1 add for sensor debug begin*/
    g_cur_active_sensor = (pDrvIndex[0] & KDIMGSENSOR_DUAL_MASK_LSB) + DUAL_CAMERA_MAIN_SENSOR;
    /*Lenovo-sw add end*/
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    PK_DBG("[CAMERA_HW] Attach I2C speed: %d\n", g_pstI2Cclient->timing );
...
/*******************************************************************************
  ...
  * Used to dump some critical sensor register
  ********************************************************************************/
/*Lenovo-sw chenglong1 modify for sensor debug begin*/
#define INVALID_REG_ADDR 0xffffffff
static int g_cur_reg_addr = INVALID_REG_ADDR;
static int g_cur_reg_val = 0;
                                                                                       ...
                                                                                       int count, int *eof, void *data)
{
    ...
    p = page;
    if (g_cur_reg_addr != INVALID_REG_ADDR) {
        p += sprintf(p, "reg[0x%04x]=0x%02x\n", g_cur_reg_addr, g_cur_reg_val);
        g_cur_reg_addr = INVALID_REG_ADDR;
    } else {
        p += sprintf(p, "\n");
    }
    *start = page + off;
                                                                     ...
                                                                     void *data)
{
    ...
    if (sscanf(regBuf, "%x %x",  &sensorReg.RegAddr, &sensorReg.RegData) == 2) {
        if (g_pSensorFunc != NULL) {
            /*Lenovo-sw chenglong1 modify for sensor debug begin*/
            g_pSensorFunc->SensorFeatureControl(g_cur_active_sensor, SENSOR_FEATURE_SET_REGISTER, (MUINT8*)&sensorReg, (MUINT32*)sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
            g_pSensorFunc->SensorFeatureControl(g_cur_active_sensor, SENSOR_FEATURE_GET_REGISTER, (MUINT8*)&sensorReg, (MUINT32*)sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
            /*Lenovo-sw modify end*/
        ...
        if (g_pSensorFunc != NULL) {
            /*Lenovo-sw chenglong1 modify for sensor debug begin*/
            g_pSensorFunc->SensorFeatureControl(g_cur_active_sensor, SENSOR_FEATURE_GET_REGISTER, (MUINT8*)&sensorReg, (MUINT32*)sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
            g_cur_reg_addr = sensorReg.RegAddr;
            g_cur_reg_val = sensorReg.RegData;
...
/*Lenovo-sw chenglong1 add for otp debug begin*/
#define OTP_DATA_MAX_LEN 4096
static int c_show(struct seq_file *m, void *v)
{
    char *pbuf = NULL;
    MUINT32 buf_len = OTP_DATA_MAX_LEN-1;

    pbuf = kmalloc(OTP_DATA_MAX_LEN, GFP_KERNEL);
    if (!pbuf) {
        printk("Buf alloc failed!\n");
        return -1;
    }

    g_pSensorFunc->SensorFeatureControl(g_cur_active_sensor, SENSOR_FEATURE_GET_OTP, (MUINT8*)pbuf, (MUINT32*)&buf_len);
...
const struct seq_operations otp_op = {
    .start  = c_start,
    .next   = c_next,
    .stop   = c_stop,
    .show   = c_show
...
{
    return seq_open(file, &otp_op);
}

static const struct file_operations proc_otp_operations = {
    .open       = otp_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = seq_release,
...
int print_sensor_reg(void *lenc, int lenclen, int rg_ratio, int bg_ratio, int rg_ratio_typical, int bg_ratio_typical, char *buf, int buflen)
{
    #define reg_print(buf, leftlen, fmt, data1, data2) do { \
      int len = 0; \
      len = sprintf(buftemp, fmt, data1, data2); \
      if (len < leftlen-1) { \
        strcpy(buf, buftemp); \
        buf += len; \
        leftlen -= len; \
    ...
    reg_print(ptr, bufleft, "========OTP_AWB========\n", 0, 0);
    reg_print(ptr, bufleft, "rg_ratio:0x%02x\n", rg_ratio, 0);
    reg_print(ptr, bufleft, "bg_ratio:0x%02x\n", bg_ratio, 0);
    reg_print(ptr, bufleft, "typical_rg:0x%02x\n", rg_ratio_typical, 0);
    reg_print(ptr, bufleft, "typical_bg:0x%02x\n", bg_ratio_typical, 0);
    reg_print(ptr, bufleft, "\n\n", 0, 0);

    reg_print(ptr, bufleft, "========OTP_LSC========\n", 0, 0);
    for (i=0; i<lenclen; i++) {
        reg_print(ptr, bufleft, "lsc[%d]:%02x\n", i, *((char*)lenc+i));
    }
}
/*Lenovo-sw add end*/
/*=======================================================================
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    /*Lenovo-sw chenglong1 add for otp debug begin*/
    prEntry = create_proc_entry("driver/otpdump", 0, NULL);
     if (prEntry) {
        prEntry->proc_fops = &proc_otp_operations;
        PK_ERR("add /proc/driver/otpdump entry successed~ \n");
     } else {
        PK_ERR("add /proc/driver/otpdump entry failed!!! \n");
     }
    /*Lenovo-sw add end*/
