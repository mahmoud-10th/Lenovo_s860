...
static int epl2182_remove(void);
...
static int epl2182_read_data_for_cali(struct i2c_client *client, HWMON_PS_STRUCT *ps_data_cali)
{
	...
	else
	{
		 ...
		 if( data_cali>8000)
		{
		    ...
		    ps_data_cali->far_away =data_cali +320;
                }
		else if((data_cali>0)&&(data_cali<1000))
		{
		    ps_data_cali->close =data_cali +460;
		    ps_data_cali->far_away =data_cali +320;
		}
...
extern int alsps_device_index ;// 0 unknow 1 avago 2 epl2182
/*----------------------------------------------------------------------------*/
static int epl2182_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
static int epl2182_local_init(void)
{
  // struct acc_hw *hw = get_cust_alsps_hw();
	...
	if(i2c_add_driver(&epl2182_i2c_driver))
	{
		APS_ERR("chenlj2 epl2182 add driver error\n");
		return -1;
	}
