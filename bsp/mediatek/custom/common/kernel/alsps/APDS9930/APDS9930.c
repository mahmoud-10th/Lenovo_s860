...
struct APDS9930_priv {
    ...
    u16         ps_level_num;
    u16         ps_value_num;
    u32         ps_level[C_CUST_PS_CALI_LEVEL];
    u32         ps_close_value[C_CUST_PS_CALI_LEVEL];
    u32         ps_far_value[C_CUST_PS_CALI_LEVEL];
    int	    ps_diff; 
...
long APDS9930_read_ps(struct i2c_client *client, u16 *data)
{
	...
	/* for lenovo board esd check solution -- liaoxl.lenovo 12.11.2012 start */
//#ifdef LENOVO_PROJECT_PRADA
//	apds_esd_check(client);
//#endif
	/* for lenovo board esd check solution -- liaoxl.lenovo 12.11.2012 end */
...
/*lenovo-sw molg1 change for phaeton 20131121 begin*/		
#if 0
	...
	else if( 0 != atomic_read(&obj->ps_psat) )
	{
		APS_DBG("ligh too high will result to failt proximiy\n");
		return 1;		
	}
#endif
/*lenovo-sw molg1 change for phaeton 20131121 end*/		
...
/*  update lux convert algo from avago FAE --liaoxl.lenovo start */
	...
	//	APS_LOG("APDS9930_eint_work rawdata ps=%d als_ch0=%d als_ch1=%d PSAT=%d als_reduce=%d!\n",obj->ps,obj->als_ch0,obj->als_ch1,atomic_read(&obj->ps_psat),obj->als_reduce);
...
/*lenovo-sw molg1 change for phaeton 20131121 begin*/
		if(data_cali < obj->ps_level[0])
		{
			ps_data_cali->close =obj->ps_close_value[0];
			ps_data_cali->far_away =obj->ps_far_value[0];

		}
		else
		{
			for(i = 1;  i < obj->ps_level_num; ++i)
			{
				if(data_cali <= obj->ps_level[i])
				{
					break;
				}
			}
			if(i >= obj->ps_level_num)
			{
				APS_ERR("le_read_data_for_cali exceed range i:%d\n", i); 
				i = obj->ps_level_num - 1;
			}
			if(obj->hw->ps_close_value[i])
			{
				ps_data_cali->close =data_cali * obj->ps_close_value[i]/10;
				ps_data_cali->far_away =data_cali * obj->ps_far_value[i]/10;
				tmp = ps_data_cali->close -ps_data_cali->far_away;
				if(tmp <obj->ps_diff)
				{
					ps_data_cali->far_away =ps_data_cali->close -obj->ps_diff;
				}				
			}
			else
			{
				return -2;
				APS_ERR("le_read_data_for_cali exceed range close:%d\n", ps_data_cali->close); 
			}
		}
/*lenovo-sw molg1 change for phaeton 20131121 end*/	
...
static int APDS9930_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->ps_level_num = sizeof(obj->hw->ps_level)/sizeof(obj->hw->ps_level[0]);
	obj->ps_value_num = sizeof(obj->hw->ps_far_value)/sizeof(obj->hw->ps_far_value[0]);
	obj->ps_diff = obj->hw->ps_min_diff;
	...
	BUG_ON(sizeof(obj->ps_level) != sizeof(obj->hw->ps_level));
	memcpy(obj->ps_level, obj->hw->ps_level, sizeof(obj->ps_level));
	BUG_ON(sizeof(obj->ps_close_value) != sizeof(obj->hw->ps_close_value));
	memcpy(obj->ps_close_value, obj->hw->ps_close_value, sizeof(obj->ps_close_value));
	BUG_ON(sizeof(obj->ps_far_value) != sizeof(obj->hw->ps_far_value));
	memcpy(obj->ps_far_value, obj->hw->ps_far_value, sizeof(obj->ps_far_value));
	/*lenovo-sw molg1 add 20140327 begin*/
