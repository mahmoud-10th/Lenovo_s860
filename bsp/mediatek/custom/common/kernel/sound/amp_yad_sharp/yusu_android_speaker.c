...
bool Speaker_Init(void)
{
   ...
   mt_set_gpio_mode(GPIO_AU_CTRL_1,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_AU_CTRL_1,GPIO_PULL_ENABLE);
   mt_set_gpio_mode(GPIO_AU_CTRL_2,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_AU_CTRL_2,GPIO_PULL_ENABLE);
   PRINTK("-Speaker_Init Success");
   return true;
}
...
void Sound_Speaker_Turnon(int channel)
{
	...
	if(gsk_on)
		return;
    mt_set_gpio_dir(GPIO_AU_CTRL_1,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_AU_CTRL_1,GPIO_OUT_ONE); // high
    mt_set_gpio_dir(GPIO_AU_CTRL_2,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_AU_CTRL_2,GPIO_OUT_ONE); // high
...
void Sound_Speaker_Turnoff(int channel)
{
	...
	if(!gsk_on)
		return;
    mt_set_gpio_dir(GPIO_AU_CTRL_1,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_AU_CTRL_1,GPIO_OUT_ZERO); // low
    mt_set_gpio_dir(GPIO_AU_CTRL_2,GPIO_DIR_OUT); // output
    mt_set_gpio_out(GPIO_AU_CTRL_2,GPIO_OUT_ZERO); // low
	gsk_on = false;
}
