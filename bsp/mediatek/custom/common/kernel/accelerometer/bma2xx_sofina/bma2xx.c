...
static bool bma_regs_br_flag = FALSE;
...
static int bma_acc_i2c_read(unsigned char chip, unsigned int adr, int addr_len, unsigned char *buf, int len)
{
	...
	unsigned short slave_addr_backup = obj->client->addr;
	int ret;

	obj->client->addr = chip;
	ret = bma_i2c_read_block(obj->client, adr, buf, len);
	if (ret)
	{
		GSE_ERR("read error: slave_addr=%02x, reg_addr=%02x, err=%d\n", chip, adr, ret);
	}
	else
	{
		GSE_LOG("read: slave_addr=%02x, reg_addr=%02x, buf[0]=0x%02x\n", chip, adr, buf[0]);
	}
	obj->client->addr = slave_addr_backup;
	return ret;
}
/*----------------------------------------------------------------------------*/
static int bma_acc_i2c_write(unsigned char chip, unsigned int adr, int addr_len ,const unsigned char *buf, int len)
{
	...
	bma_i2c_write_block(obj->client, adr, buf, len);
	if (ret)
	{
		GSE_ERR("write error: slave_addr=%02x, reg_addr=%02x, err=%d\n", chip, adr, ret);
	}
	else
	{
		GSE_LOG("write: slave_addr=%02x, reg_addr=%02x\n", chip, adr);
	}
...
static int bma_fs_read(const unsigned char * name, unsigned char * buf, int size)
{
	...
	if (err != size) 
	{
		GSE_ERR("%s: Can't read the data from file:%s\n", __func__,name);
		err = -EIO;
	}
...
static int bma_fs_write(const unsigned char * name, unsigned char * buf, int size)
{
	...
	if (IS_ERR(p_fs)) 
	{
		GSE_ERR("%s: Can't open file %s error=%d\n", __func__, name,p_fs);
		set_fs(old_fs);
		err = PTR_ERR(p_fs);
		return err;
	}
	...
	if (err != size) 
	{
		GSE_ERR("%s: Can't write the data to file:%s\n", __func__,name);
		err = -EIO;
	}
...
int backup_or_restore_i2c( struct bma_callback * pCB)
{
	...
	{ 
		...
		if (!read_from_nvm)
		{
			GSE_ERR("cannot get saved data from NVM\n");					
			return -1;
		}
		...
		if (crc8 != regs[0x1A])
		{
			GSE_ERR("Restore CRC check failed , crc8 = 0x%x regs[0x1A] = 0x%x\n",crc8,regs[0x1A]);
			return -1;
		}
	...
	else
	{
		...
		if (crc8 == regs[0x1A])
		{
			...
			{
				GSE_ERR("backup to NVM failed\n");
				return -1;
			}
		}
		else
		{
			GSE_ERR("Backup CRC check failed , crc8 = 0x%x regs[0x1A] = 0x%x\n",crc8,regs[0x1A]);
			return -1;				
		}
	}
		...
		const char *buf, size_t count)
{
	...
	GSE_LOG("store_cali_value start:%s\n", buf);
	if (NULL == obj) {
		GSE_ERR("bma i2c data pointer is null\n");
		return 0;
	}
		...
		unsigned long arg)
{
	...
	case GSENSOR_IOCTL_INIT:
		GSE_ERR("GSENSOR_IOCTL_INIT: 0x%08x\n", cmd);
	...
	case GSENSOR_IOCTL_READ_CHIPINFO:
		GSE_ERR("GSENSOR_IOCTL_READ_CHIPINFO: 0x%08x\n", cmd);
	...
	case GSENSOR_IOCTL_READ_SENSORDATA:
		GSE_ERR("GSENSOR_IOCTL_READ_SENSORDATA: 0x%08x\n", cmd);
	data = (void __user *) arg;
	if (data == NULL) {
		err = -EINVAL;
		break;
	...
	case GSENSOR_IOCTL_READ_GAIN:
		GSE_ERR("GSENSOR_IOCTL_READ_GAIN: 0x%08x\n", cmd);
	data = (void __user *) arg;
	if (data == NULL) {
		err = -EINVAL;
		break;
	...
	case GSENSOR_IOCTL_READ_RAW_DATA:
		GSE_ERR("GSENSOR_IOCTL_READ_RAW_DATA: 0x%08x\n", cmd);
	data = (void __user *) arg;
	if (data == NULL) {
		err = -EINVAL;
		break;
	...
	case GSENSOR_IOCTL_SET_CALI:
		GSE_ERR("GSENSOR_IOCTL_SET_CALI: 0x%08x\n", cmd);
	/* data unit is m/s^2 */
	data = (void __user *)arg;
	if (data == NULL) {
		err = -EINVAL;
	...
	case GSENSOR_IOCTL_CLR_CALI:
		GSE_ERR("GSENSOR_IOCTL_CLR_CALI: 0x%08x\n", cmd);
	err = bma_reset_calibration(client);
	break;
	case GSENSOR_IOCTL_GET_CALI:
		GSE_ERR("GSENSOR_IOCTL_GET_CALI: 0x%08x\n", cmd);
	data = (void __user *)arg;
	if (data == NULL) {
		err = -EINVAL;
		break;
		...
		const struct i2c_device_id *id)
{
	...
	struct bma_callback bc = {
		...
		.msleep 	=	msleep,
		.sensor_i2c_id	= BMA_I2C_ADDRESS(BMA2X2, PIN_LOW),
	};
#endif //BMA_REGS_BACKUP_RESTORE
	GSE_FUN();
...
#ifdef BMA_REGS_BACKUP_RESTORE
	if (detect_bma_sensor(&bc) == 0)
	{
		err = backup_or_restore_i2c(&bc);
		if (err == 0)
		{
			GSE_LOG("bma regs backup or restore successfully\n");
		}
		else
		{
			GSE_ERR("bma regs backup or restore failed\n");
		}
	}
	else
	{
	...
	if (err){
		GSE_ERR("init client failed, err = %d\n", err);
...
static int bma_probe(struct platform_device *pdev)
{
	...
	bma_power(hw, 1);
GSE_LOG("xmr bma power end\n");
	if (i2c_add_driver(&bma_i2c_driver)) {
		GSE_LOG("add i2c driver failed!!!\n");
		return -1;
	}
GSE_LOG("xmr bma probe end\n");
	return 0;
}
