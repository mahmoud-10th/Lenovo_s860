...
static OV9760_sensor_struct OV9760_sensor =
{
  .eng =
  {
    .reg = OV9760_CAMERA_SENSOR_REG_DEFAULT_VALUE,
    .cct = OV9760_CAMERA_SENSOR_CCT_DEFAULT_VALUE,
  },
  .eng_info =
  {
    .SensorId = 128,
...
static int check_otp_wb(int index)
{
           ...
           // select bank index
           bank = 0xc0 | index;
           OV9760MIPI_write_cmos_sensor(0x3d84, bank);
           // read otp into buffer
           OV9760MIPI_write_cmos_sensor(0x3d81, 0x01);
           Sleep(5);
           // read flag
           address = 0x3d00;
           flag = OV9760MIPI_read_cmos_sensor(address);
           flag = flag & 0xc0;
           // clear otp buffer
           for (i=0;i<16;i++) {
                        OV9760MIPI_write_cmos_sensor(0x3d00 + i, 0x00);
           }
           if (flag == 0x00) {
                        return 0;
           }
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
           ...
           Sleep(5);
           address = 0x3d00;
           (*otp_ptr).module_integrator_id = OV9760MIPI_read_cmos_sensor(address + 1);
           (*otp_ptr).lens_id = OV9760MIPI_read_cmos_sensor(address + 2);
           (*otp_ptr).production_year = OV9760MIPI_read_cmos_sensor(address + 3);
           (*otp_ptr).production_month = OV9760MIPI_read_cmos_sensor(address + 4);
           (*otp_ptr).production_day = OV9760MIPI_read_cmos_sensor(address + 5);
           temp = OV9760MIPI_read_cmos_sensor(address + 10);
           (*otp_ptr).rg_ratio = (OV9760MIPI_read_cmos_sensor(address + 6)<<2) + ((temp>>6) & 0x03);
           (*otp_ptr).bg_ratio = (OV9760MIPI_read_cmos_sensor(address + 7)<<2) + ((temp>>4) & 0x03);
           (*otp_ptr).typical_rg = (OV9760MIPI_read_cmos_sensor(address + 8) <<2) + ((temp>>2) & 0x03);
           (*otp_ptr).typical_bg = (OV9760MIPI_read_cmos_sensor(address + 9)<<2) + (temp & 0x03);
           (*otp_ptr).user_data[0] = OV9760MIPI_read_cmos_sensor(address + 11);
           (*otp_ptr).user_data[1] = OV9760MIPI_read_cmos_sensor(address + 12);
           (*otp_ptr).user_data[2] = OV9760MIPI_read_cmos_sensor(address + 13);
           (*otp_ptr).user_data[3] = OV9760MIPI_read_cmos_sensor(address + 14);
           (*otp_ptr).user_data[4] = OV9760MIPI_read_cmos_sensor(address + 15);
...
static int read_otp_lenc(int index, struct otp_struct *otp_ptr)
{
           ...
           Sleep(5);
           address = 0x3d01;
           temp = OV9760MIPI_read_cmos_sensor(address + 2);
           (*otp_ptr).lenc[0] = (temp & 0x39)>>3;                            // Red_X0[10:8]
           (*otp_ptr).lenc[1] = OV9760MIPI_read_cmos_sensor(address);                    // Red_X0[7:0]
           (*otp_ptr).lenc[2] = temp & 0x07;                                 // Red_Y0[10;8}
          (*otp_ptr).lenc[3] = OV9760MIPI_read_cmos_sensor(address + 1);   // Red_Y0[7:0]
          (*otp_ptr).lenc[4] = OV9760MIPI_read_cmos_sensor(address + 3);   // Red_A1
          temp = OV9760MIPI_read_cmos_sensor(address + 5);
          (*otp_ptr).lenc[5] = temp>>4;                        // Red_A2
          (*otp_ptr).lenc[6] = OV9760MIPI_read_cmos_sensor(address + 4);   // Red_B1
          (*otp_ptr).lenc[7] = temp & 0x0f;                    // Red_B2
          temp = OV9760MIPI_read_cmos_sensor(address + 8);
          (*otp_ptr).lenc[8] = (temp & 0x39)>>3;               // Green_X0[10:8]
          (*otp_ptr).lenc[9] = OV9760MIPI_read_cmos_sensor(address + 6);   // Green_X0[7:0]
          (*otp_ptr).lenc[10] = temp & 0x07;                   // Green_Y0[10;8}
          (*otp_ptr).lenc[11] = OV9760MIPI_read_cmos_sensor(address + 7);  // Green_Y0[7:0]
          (*otp_ptr).lenc[12] = OV9760MIPI_read_cmos_sensor(address + 9);  // Green_A1
          temp = OV9760MIPI_read_cmos_sensor(address + 11);
          (*otp_ptr).lenc[13] = temp>>4;                       // Green_A2
          (*otp_ptr).lenc[14] = OV9760MIPI_read_cmos_sensor(address + 10); // Green_B1
          (*otp_ptr).lenc[15] = temp & 0x0f;                   // Green_B2
          temp = OV9760MIPI_read_cmos_sensor(address + 14);
          (*otp_ptr).lenc[16] = (temp & 0x39)>>3;              // Blue_X0[10:8]
          (*otp_ptr).lenc[17] = OV9760MIPI_read_cmos_sensor(address + 12); // Blue_X0[7:0]
          (*otp_ptr).lenc[18] = temp & 0x07;                   // Blue_Y0[10;8}
          (*otp_ptr).lenc[19] = OV9760MIPI_read_cmos_sensor(address + 13); // Blue_Y0[7:0]
          bank ++;
          OV9760MIPI_write_cmos_sensor(0x3d84, bank);
          // read otp into buffer
          OV9760MIPI_write_cmos_sensor(0x3d81, 0x01);
          address = 0x3d00;
          (*otp_ptr).lenc[20] = OV9760MIPI_read_cmos_sensor(address);      // Blue_A1
          temp = OV9760MIPI_read_cmos_sensor(address + 2);
          (*otp_ptr).lenc[21] = temp>>4;                       // Blue_A2
          (*otp_ptr).lenc[22] = OV9760MIPI_read_cmos_sensor(address + 1);  // Blue_B1
...
{
	int isp_ctrl2 = 0;
	SENSORDB("OV9760DB_update_awb_gain ENTER :\n ");

	SENSORDB("OV9760DB_update_awb_gain R_gain =0x%x, G_gain=0x%x, B_gain=0x%x\n",R_gain, G_gain, B_gain);
    if (R_gain>0x400) {
		OV9760MIPI_write_cmos_sensor(0x5180, R_gain>>8);
		OV9760MIPI_write_cmos_sensor(0x5181, R_gain & 0x00ff);
    }
    if (G_gain>0x400) {
		OV9760MIPI_write_cmos_sensor(0x5182, G_gain>>8);
		OV9760MIPI_write_cmos_sensor(0x5183, G_gain & 0x00ff);
    }
    if (B_gain>0x400) {
		OV9760MIPI_write_cmos_sensor(0x5184, B_gain>>8);
		OV9760MIPI_write_cmos_sensor(0x5185, B_gain & 0x00ff);
    }
	//add otp awb enable
	isp_ctrl2 = OV9760MIPI_read_cmos_sensor(0x5002);
	isp_ctrl2 = isp_ctrl2 |0x01;
	OV9760MIPI_write_cmos_sensor(0x5002, isp_ctrl2);
    return 0;
}
...
static int force_read_otp_wb(struct otp_struct *current_otp)
{
  ...
  if (i>3) {
    SENSORDB("[OV9760OTP]No WB OTP Data\n");
    // no valid wb OTP data
    return -1;
  }

  return read_otp_wb(otp_index, current_otp);
}
...
if(bg < BG_Ratio_Typical) {
          ...
          if(G_gain_B > G_gain_R ) {
            ...
            SENSORDB("bg =0x%x, rg=0x%x,RG_Ratio_Typical=0x%x,BG_Ratio_Typical=0x%x\n",bg, rg,RG_Ratio_Typical,BG_Ratio_Typical);
...
{
            int i, temp;
            temp = OV9760MIPI_read_cmos_sensor(0x5000);
            temp = 0x80 | temp;
            OV9760MIPI_write_cmos_sensor(0x5000, temp);
            for(i=0;i<24;i++) {
            					SENSORDB("OV9760OTP ADRESS:=0x%x, PARA:=0x%x\n ",0x5800 + i,(*otp_ptr).lenc[i]);
                       OV9760MIPI_write_cmos_sensor(0x5800 + i, (*otp_ptr).lenc[i]);
            }
            return 0;
}
...
int update_blc_ratio()
{
            int K;
            int temp;
            OV9760MIPI_write_cmos_sensor(0x3d84, 0xdf);
            OV9760MIPI_write_cmos_sensor(0x3d81, 0x01);
            Sleep(5);
            K = OV9760MIPI_read_cmos_sensor(0x3d0b);
            if (K != 0) {
                        if (K >= 0x15 && K <= 0x40) {
                                   // auto load mode
                                   temp = OV9760MIPI_read_cmos_sensor(0x4000);
                                   temp &= 0x9f;
                                   OV9760MIPI_write_cmos_sensor(0x4000, temp);
                                   return 2;
                        }
            }
            K = OV9760MIPI_read_cmos_sensor(0x3d0a);
            if (K >= 0x10 && K <= 0x40) {
                        // manual load mode
                        OV9760MIPI_write_cmos_sensor(0x4006, K);
                        temp = OV9760MIPI_read_cmos_sensor(0x4000);
                        temp |= 0x40;
                        temp &= 0xdf;
                        OV9760MIPI_write_cmos_sensor(0x4000, temp);
                        ...
                        // set to default
                        OV9760MIPI_write_cmos_sensor(0x4006, 0x20);
                        temp = OV9760MIPI_read_cmos_sensor(0x4000);
                        temp |= 0x40;
                        temp &= 0xdf;
                        OV9760MIPI_write_cmos_sensor(0x4000, temp);
...
void OV9760MIPI_Set720P(void)
{
	...
	OV9760MIPI_write_cmos_sensor(0x3021, 0x20); //zhangjiano close int dvdd
	...
	OV9760MIPI_write_cmos_sensor(0x0343, 0xfa); //";HTS, 03/05/2012"
	...
	OV9760MIPI_write_cmos_sensor(0x4800, 0x54); // mipi short p. 0x44

	OV9760MIPI_write_cmos_sensor(0x5781, 0x17); // zhangjiano add
	OV9760MIPI_write_cmos_sensor(0x5792, 0x00); // zhangjiano add 
	OV9760MIPI_write_cmos_sensor(0x5000,  0x86); // lenc on
