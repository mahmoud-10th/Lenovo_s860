...
#include "ov8825sunnymipiraw_Sensor.h"
#include "ov8825sunnymipiraw_Camera_Sensor_para.h"
#include "ov8825sunnymipiraw_CameraCustomized.h"
static DEFINE_SPINLOCK(ov8825mipirawsunny_drv_lock);

#define OV8825_DEBUG
#define OV8825_DEBUG_SOFIA

#ifdef OV8825_DEBUG
	#define OV8825DB(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV8825MIPI_SUNNY]" , fmt, ##arg)
#else
	#define OV8825DB(x,...)
#endif

#ifdef OV8825_DEBUG_SOFIA
	#define OV8825DBSOFIA(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV8825MIPI_SUNNY]", fmt, ##arg)
#else
...
static void OV8825_write_shutter(kal_uint32 shutter)
{
	...
	if(ov8825.OV8825AutoFlickerMode == KAL_TRUE)
	{
		...
		spin_lock_irqsave(&ov8825mipirawsunny_drv_lock,flags);
		...
		spin_unlock_irqrestore(&ov8825mipirawsunny_drv_lock,flags);
...
static void OV8825_Sensor_Init(void)
{
	...
	mdelay(5);
	OV8825_write_cmos_sensor(0x3608,0x40);//zhangjiano close int dvdd
...
static UINT32 OV8825Open(void)
{
    ...
    UINT32 check_mid=0;
	OV8825DB("OV8825SUNNYOpen enter :\n ");
	...
	for(i=0;i<3;i++)
	{
		...
		check_mid=OV8825_OTP_check_mid();//read mid in open API
		(sensor_id)+=1;//#lenovo.sw START wangsx3 merge kk
		OV8825DB("OOV8825OPEN READ ID :%x",sensor_id);
		if((sensor_id != OV8825SUNNY_SENSOR_ID) || (!check_mid))
		{
...
static int read_otp_mid_lensid(int index)
{
  ...
  OTP_SENSOR_MID=OV8825_read_cmos_sensor(address);//sunnny =0x01,,qtech=0x06
  OTP_SENSOR_LENSID=OV8825_read_cmos_sensor(address + 1);
  //disable otp read
  OV8825_write_cmos_sensor(0x3d81, 0x00);
  // clear otp buffer
  clear_otp_buffer();
  printk("%s SUNNY MID=0x%02x LENSID=0x%02x\r\n",__func__,OTP_SENSOR_MID,OTP_SENSOR_LENSID);
  return 0;
}
...
{
  kal_int8 mid = 0x00;
  printk("OV8825_OTP_check_mid SUNNY\n");    
  OV8825_write_cmos_sensor(0x0103,0x01);// reset sensor
  Sleep(5);
  ...
  if (OTP_SENSOR_MID==0x01) //sunnny =0x01,,qtech=0x06
  {
    return TRUE;
  }
  else
...
static UINT32 OV8825GetSensorID(UINT32 *sensorID)
{
  ...
  do {
	...
	(*sensorID)+=1;//#lenovo.sw START wangsx3 kk merge
      if (*sensorID == OV8825SUNNY_SENSOR_ID)
      {
	...
	check_mid=OV8825_OTP_check_mid();
        ...
        {
          *sensorID = 0xFFFFFFFF;
          OV8825DB("OV8825Get Sensor ID, MID != 0x01 \n");
          return ERROR_SENSOR_CONNECT_FAIL;
        }
        break;
      }
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                pSensorDefaultData->SensorId=OV8825SUNNY_SENSOR_ID;
...
UINT32 OV8825SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
