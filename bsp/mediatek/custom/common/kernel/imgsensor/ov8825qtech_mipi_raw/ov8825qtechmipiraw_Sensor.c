...
#include "ov8825qtechmipiraw_Sensor.h"
#include "ov8825qtechmipiraw_Camera_Sensor_para.h"
#include "ov8825qtechmipiraw_CameraCustomized.h"
static DEFINE_SPINLOCK(ov8825mipirawqtech_drv_lock);

#define OV8825_DEBUG
#define OV8825_DEBUG_SOFIA

#ifdef OV8825_DEBUG
	#define OV8825DB(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV8825MIPI_OFLIM]" , fmt, ##arg)
#else
	#define OV8825DB(x,...)
#endif

#ifdef OV8825_DEBUG_SOFIA
	#define OV8825DBSOFIA(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV8825MIPI_OFLIM]", fmt, ##arg)
#else
...
static kal_uint32 OV8825_FeatureControl_PERIOD_PixelNum=OV8825_PV_PERIOD_PIXEL_NUMS;
static kal_uint32 OV8825_FeatureControl_PERIOD_LineNum=OV8825_PV_PERIOD_LINE_NUMS;

static UINT16 VIDEO_MODE_TARGET_FPS = 30;
static BOOL ReEnteyCamera = KAL_FALSE;


static MSDK_SENSOR_CONFIG_STRUCT OV8825SensorConfigData;

static kal_uint32 OV8825_FAC_SENSOR_REG;

static MSDK_SCENARIO_ID_ENUM OV8825CurrentScenarioId = MSDK_SCENARIO_ID_CAMERA_PREVIEW;

/* FIXME: old factors and DIDNOT use now. s*/
static SENSOR_REG_STRUCT OV8825SensorCCT[]=CAMERA_SENSOR_CCT_DEFAULT_VALUE;
static SENSOR_REG_STRUCT OV8825SensorReg[ENGINEER_END]=CAMERA_SENSOR_REG_DEFAULT_VALUE;
/* FIXME: old factors and DIDNOT use now. e*/
/*lenovo.sw 20131030 wangsx3 add for otp mid 2013_10_18 beging*/
static int OTP_SENSOR_MID=0;
static int OTP_SENSOR_LENSID=0;
static UINT32 OV8825_OTP_check_mid(void);
/*lenovo.sw 20131030 wangsx add for otp mid 2013_10_18 end*/
...
static kal_uint16 OV8825_read_cmos_sensor(kal_uint32 addr)
{
...
static void OV8825_write_shutter(kal_uint32 shutter)
{
	...
	if(ov8825.OV8825AutoFlickerMode == KAL_TRUE)
	{
		...
		spin_lock_irqsave(&ov8825mipirawqtech_drv_lock,flags);
		...
		spin_unlock_irqrestore(&ov8825mipirawqtech_drv_lock,flags);
...
static void write_OV8825_gain(kal_uint16 gain)
{
...
*
*************************************************************************/
static void OV8825_SetGain(UINT16 iGain)
{
	unsigned long flags;
	spin_lock_irqsave(&ov8825mipirawqtech_drv_lock,flags);
...
static void OV8825_camera_para_to_sensor(void)
{
    kal_uint32    i;
...
*
*************************************************************************/
static void OV8825_sensor_to_camera_para(void)
{
...
*
*************************************************************************/
static kal_int32  OV8825_get_sensor_group_count(void)
{
    return GROUP_TOTAL_NUMS;
}

static void OV8825_get_sensor_group_info(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
        case PRE_GAIN:
...
static void OV8825_get_sensor_item_info(kal_uint16 group_idx,kal_uint16 item_idx, MSDK_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
...
static kal_bool OV8825_set_sensor_item_info(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
//   kal_int16 temp_reg;
   kal_uint16  temp_gain=0,temp_addr=0, temp_para=0;
...
#define GOLDEN_RG_RATIO 0x57  //R/G 85.95  =86=0x56
#define GOLDEN_BG_RATIO 0x56  //B/G  88.67 =89=0x59
//*******golden sample value: module vendor provide********//
...
static kal_bool OV8825OTPWriteSensor(kal_uint16 address, kal_uint16 para)
{
	 OV8825_write_cmos_sensor(address, para);	
}
	
static kal_uint16 OV8825OTPReadSensor(kal_uint16 address)
{
	kal_uint16 reVal;
...
static u16 OV8825_update_awb_gain(u16 R_gain, u16 G_gain, u16 B_gain)
{
...
static u8 OV8825_update_otp_wb(u8 Cur_RG_ratio,u8 Cur_BG_ratio)
{
...
static kal_bool OV8825_ReadOtp(u16 Bank,u16 address,unsigned char*iBuffer,u16 buffersize)
{
...
static kal_uint8 check_OV8825_otp_flag(u16 address)
{
	u16 reVal;
...
//read LSC parameter
static kal_bool OV8825OTPShading(void)
{
	...
	//lenovo.sw 20131227 wangsx3 fix green circle issue
	OV8825OTPWriteSensor(OV8825_SHADING_VALUE_ADD+14,lscDecode[14]+1);
	OV8825OTPWriteSensor(OV8825_SHADING_VALUE_ADD+15,lscDecode[15]+1);
	OV8825OTPWriteSensor(OV8825_SHADING_VALUE_ADD+20,lscDecode[20]+1);
	OV8825OTPWriteSensor(OV8825_SHADING_VALUE_ADD+21,lscDecode[21]+1);
...
//read AWB data
static kal_bool OV8825OTPWbPregain(void)
{
...
static void OV8825PreviewSettingOTP(void)
{	
	...
	OV8825DB("OV8825PreviewSetting_4lane_30fps enter ob versioin:\n ");
...
static void OV8825PreviewSetting(void)
{
...
static void OV8825VideoSetting(void)
{
    if(ReEnteyCamera == KAL_TRUE)
    {
		OV8825_write_cmos_sensor(0x0100, 0x00);
	...
	OV8825DB("OV8825VideoSetting/6M_16:9 enter ob Version :\n ");
...
static void OV8825CaptureSetting(void)
{
    if(ReEnteyCamera == KAL_TRUE)
    {
		OV8825_write_cmos_sensor(0x0100, 0x00);
	...
	else
	{
		...
		OV8825DB("OV8825CaptureSetting_4lane_24fps enter OB version :\n ");
		...
		OV8825_write_cmos_sensor(0x3f01,0xfc);//;PSRAM Ctrl1			  
		OV8825_write_cmos_sensor(0x3f05,0x10);//;PSRAM Ctrl5 
		OV8825_write_cmos_sensor(0x4005,0x18);//lenovo.sw wangsx3 20131128 change blc state,set to freezen
		OV8825_write_cmos_sensor(0x4600,0x04);//;VFIFO Ctrl0			  
	...
	ReEnteyCamera = KAL_FALSE;
	OV8825DB("OV8825CaptureSetting_4lane_24fps exit :\n ");
}
...
static void OV8825_Sensor_Init(void)
{
	...
	OV8825DB("OV8825_Sensor_Init enter_4lane OB_Version:\n ");	
	...
	OV8825_write_cmos_sensor(0x6900,0x60);//
	OV8825_write_cmos_sensor(0x6901,0x05);//; CADC control
	//Lens Control
...
static UINT32 OV8825Open(void)
{

	volatile signed int i;
	kal_uint16 sensor_id = 0;
    UINT32 check_mid=0;
	OV8825DB("OV8825OFLIMOpen enter :\n ");
	...
	for(i=0;i<3;i++)
	{
		...
		check_mid=OV8825_OTP_check_mid();
		(sensor_id)+=2;//#lenovo.sw START wangsx3 merge kk
		OV8825DB("OOV8825OPEN READ ID :%x",sensor_id);
		if((sensor_id != OV8825QTECH_SENSOR_ID) || (!check_mid))
		{
...
/*lenovo.sw START 20131101 wangsx3 add for otp mid 2013_10_18 beging*/
//-----------------------------------------------------//
static clear_otp_buffer()
{
  int i=0;
  for (i=0;i<32;i++)
  {
    OV8825_write_cmos_sensor(0x3d00 + i, 0x00);
  }
}
...
static int check_otp_mid_lensid(int index)
{
  int flag, i;
  int bank, address;
  // select bank 0
  ...
  //flag = flag & 0xc0;
  OV8825DB("%s index=%d,flag=%d,address=%x\n",__func__,index,flag,address);
  // clear otp buffer
  clear_otp_buffer();
  if (flag == 0x00)
  {
...
// return: 0,
static int read_otp_mid_lensid(int index)
{
  int i, bank;
  int address;
  int temp;
  ...
  Sleep(3);
  //read flag
  address = 0x3d05+(index-1)*9;//+index*9;
  OTP_SENSOR_MID=OV8825_read_cmos_sensor(address);//sunnny =0x01,,OFLIM=0x07
  OTP_SENSOR_LENSID=OV8825_read_cmos_sensor(address + 1);
  //disable otp read
  OV8825_write_cmos_sensor(0x3d81, 0x00);
  // clear otp buffer
  clear_otp_buffer();
  printk("%s OFLIM MID=0x%02x LENSID=0x%02x\r\n",__func__,OTP_SENSOR_MID,OTP_SENSOR_LENSID);
  return 0;
}
...
//                     1, no OTP
static int update_otp_mid_lensid()
{
  int i;
  int otp_index=0;
  int temp=0;
  ...
  if (i>3) 
  {
    ...
    OV8825DB("%s no MID and LENSID data\n",__func__);
    return 1;
  }
  read_otp_mid_lensid(otp_index);
  return 0;
}
...
{
  kal_int8 mid = 0x00;
  printk("OV8825_OTP_check_mid OFLIM\n");    
  OV8825_write_cmos_sensor(0x0103,0x01);// reset sensor
  Sleep(5);
  OV8825_write_cmos_sensor(0x0100,0x00);// strame off
  Sleep(5);
  OV8825_write_cmos_sensor(0x0100,0x01);// strame on
  Sleep(5);
  update_otp_mid_lensid();
  OV8825_write_cmos_sensor(0x0100,0x00);// strame off

  printk("%s OFLIM MID=0x%02x LENSID=0x%02x\r\n",__func__,OTP_SENSOR_MID,OTP_SENSOR_LENSID);

  if (OTP_SENSOR_MID==0x07) //sunnny =0x01,,OFLIM=0x07
  {
    return TRUE;
  }
  else
...
/*lenovo.sw END 20131101 wangsx3 add for otp mid 2013_10_18 beging*/
/*************************************************************************
* FUNCTION
*   OV8825GetSensorID
*
...
*
*************************************************************************/
static UINT32 OV8825GetSensorID(UINT32 *sensorID)
{
  int  retry = 1;
  UINT32 check_mid=0;
  ...
  do {
      *sensorID = (OV8825_read_cmos_sensor(0x300A)<<8)|OV8825_read_cmos_sensor(0x300B);
      (*sensorID)+=2;//#lenovo.sw START wangsx3 merge kk
      if (*sensorID == OV8825QTECH_SENSOR_ID)
      {
        OV8825DB("Get Sensor ID = 0x%04x\n", *sensorID);
	check_mid=OV8825_OTP_check_mid();
        if (!check_mid)
        {
          *sensorID = 0xFFFFFFFF;
          OV8825DB("OV8825Get Sensor ID, MID != 0x06 \n");
          return ERROR_SENSOR_CONNECT_FAIL;
        }
        break;
      }
...
*
*************************************************************************/
static void OV8825_SetShutter(kal_uint32 iShutter)
{
...
*
*************************************************************************/
static UINT32 OV8825_read_shutter(void)
{
...
*
*************************************************************************/
static void OV8825_NightMode(kal_bool bEnable)
{
}/*	OV8825_NightMode */
...
*
*************************************************************************/
static UINT32 OV8825Close(void)
{
...
static void OV8825SetFlipMirror(kal_int32 imgMirror)
{
...
*
*************************************************************************/
static UINT32 OV8825Preview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
...
static UINT32 OV8825Video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
static UINT32 OV8825Capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
...
static UINT32 OV8825GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
...
static UINT32 OV8825GetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pSensorInfo->MIPIsensorType = MIPI_OPHY_CSI2; //zhangjiano modify BB mipi
...
static UINT32 OV8825Control(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		spin_lock(&ov8825mipirawqtech_drv_lock);
...
static UINT32 OV8825SetVideoMode(UINT16 u2FrameRate)
{
...
static UINT32 OV8825SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
	//return ERROR_NONE;
...
static UINT32 OV8825SetTestPatternMode(kal_bool bEnable)
{
...
static UINT32 OV8825MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
{
	kal_uint32 pclk;
	kal_int16 dummyLine;
...
static UINT32 OV8825MIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
...
static UINT32 OV8825FeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                pSensorDefaultData->SensorId=OV8825QTECH_SENSOR_ID;
...
static SENSOR_FUNCTION_STRUCT	SensorFuncOV8825=
{
    OV8825Open,
    OV8825GetInfo,
    OV8825GetResolution,
...
UINT32 OV8825QTECH_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
