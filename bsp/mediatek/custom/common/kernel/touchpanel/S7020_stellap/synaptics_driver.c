...
//#include "SynaImage_014A0008.h"
#include "SynaImage_014A000A.h"
...
#ifdef MUTI_CORE_CONTROL
static int tpd_boost_cpu_cores = 8;
#endif
...
static void boost_cpu_work_func(struct work_struct *work)
{
	...
	if (tpd_boost_cpu_cores > 8) tpd_boost_cpu_cores = 8;
...
static void horizontal_slide(int x, int y, int p, int Xw, int Yw)
#else
static void horizontal_slide(int x, int y, int p)
#endif
{
    int i = 0;
    ktime_t ktime;

    printk("[wj]setting_y is %d .\n", setting_y);
    if((x < area_x) && (y < area_y))
    {
        ...
        {
            #if defined(LENOVO_AREA_TOUCH)
            tpd_down(i, setting_y, p, Xw, Yw);
            #else
            tpd_down(i, setting_y, p);
            #endif
            input_sync(tpd->dev);
        }
        for(i = 50; i < LCD_X; i+=coordinate_interval)
        {
            #if defined(LENOVO_AREA_TOUCH)
            tpd_down(i, setting_y, p, Xw, Yw);
            #else
            ...
            #endif
            input_sync(tpd->dev);
            //ktime = ktime_set(0, TIMER_MS_TO_NS(delay_time));	
	        //hrtimer_start(&tpd_test_fluency_timer, ktime, HRTIMER_MODE_REL);
            mdelay(delay_time);
        }
        tpd_up((LCM_WIDTH-1), setting_y);
        input_sync(tpd->dev);
        mdelay(10);
    }            
}
...
#if defined(LENOVO_AREA_TOUCH)
static void vertical_slide(int x, int y, int p, int Xw, int Yw)
#else
static void vertical_slide(int x, int y, int p)
#endif
{
    int i = 0;
    if((x < area_x) && (y < area_y))
    {
        ...
        for(i = LCM_HEIGHT; i > LCD_Y - 50; i -= 5)
        {
            #if defined(LENOVO_AREA_TOUCH)
            tpd_down(setting_x, i, p, Xw, Yw);
            #else
            tpd_down(setting_x, i, p);
            #endif
            input_sync(tpd->dev);
        }
        for(i = LCD_Y - 50; i > 0; i-=coordinate_interval)
        {
            #if defined(LENOVO_AREA_TOUCH)
            tpd_down(setting_x, i, p, Xw, Yw);
            #else
        ...
        if (status & ts->fn1a_mask) { /* key */
                ...
                if (button)
                {
                        ...
                        for (i = 0; i < MAX_KEY_NUM; i++)
                        {
                              if ((button & (0x01 << i)) == 1)
								{
									printk("[wj]input_report_key touch_key_array[0] is %d ...\n", touch_key_array[0]);
                              ...
                              if ((button & (0x01 << i)) == 2)
								{
									printk("[wj]input_report_key touch_key_array[1] is %d ...\n", touch_key_array[1]);
                              ...
                              if ((button & (0x01 << i)) == 4)
								{
									printk("[wj]input_report_key touch_key_array[2] is %d ...\n", touch_key_array[2]);
...
//end add for slide
                ...
                for (i = 0; i < ts->points_supported; i++) {
                        ...
                        if (0x01==finger_status || 0x02 == finger_status) {
                            ...
                            {
                                #if defined(LENOVO_AREA_TOUCH)
                                horizontal_slide(ppt->x, ppt->y, ppt->z, XYsqr, XYsqr);
                                #else
                                horizontal_slide(ppt->x, ppt->y, ppt->z);
                                #endif
                            }
                            else
                            {
                                #if defined(LENOVO_AREA_TOUCH)
                                vertical_slide(ppt->x, ppt->y, ppt->z, XYsqr, XYsqr);
                                #else
                                vertical_slide(ppt->x, ppt->y, ppt->z);
                                #endif            
                            }
                            #endif
...
/*lenovo-sw xuwen1 add 20140127 for read ID&FW version begin*/

unsigned int tpd_read_id_version(void)
{
       int i;
  	unsigned char ver[4];
	unsigned int ver_no = 0;
	int retval;
	retval = tpd_i2c_read_data(ts->client,0x0078,ver,sizeof(ver));//0x00AE
       if (retval < 0) {
		TPD_DMESG("Failed to read id (code %d).\n", retval);
		return retval;
	}
	   for(i = 0; i < sizeof(ver); i++)
	{
		printk("[TSP-xw]ver[%d] version = 0x%02x\n", i, ver[i]);
		ver_no += ((int)ver[i] << ((sizeof(ver) - i - 1) * 8));
	}
	return(ver[3]);
}
...
tpd_info_t ->types = 0x01;//tpd_read_id_version();

have_correct_setting = 1;
}
/*lenovo-sw xuwen1 add 20131212 for read ID version end*/
...
#endif
	{
		printk("[wj]%s, no enter resume lpwg.\n", __func__);
		lpwg_handler.supported = false;
	}
	if (lpwg_handler.supported) {
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	{
		printk("[wj]%s, enter suspend lpwg.\n", __func__);
		lpwg_handler.supported = true;
		lpwg_flag = 1;
		lpwg_int_flag = 1;
	}
...
#endif
	{
		printk("[wj]%s, no enter suspend lpwg.\n", __func__);
		lpwg_handler.supported = false;
	}
	if (lpwg_handler.supported) {
