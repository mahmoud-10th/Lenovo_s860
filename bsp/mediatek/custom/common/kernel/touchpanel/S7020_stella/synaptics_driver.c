...
/*lenovo-sw xuwen1 add 20140123 for read ID version begin*/
...
unsigned int tpd_read_id_version(void)
{
	...
	int retval;
	retval = tpd_i2c_read_data(ts->client,0x0078,ver,sizeof(ver));//PVT changed to 0x0078
       if (retval < 0) {
...
tpd_info_t ->types = 0x01;

have_correct_setting = 1;
}

/*lenovo-sw xuwen1 add 20140123 for read ID version end*/
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   TPD_DMESG("%s:enter \n",__func__);
        u8 ii;
