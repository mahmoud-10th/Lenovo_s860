...
#define BD7710_STRB0     GPIO_CAMERA_FLASH_EN_PIN//PIN: FLASH_HWEN, GPIO115
#define BD7710_STRB1     GPIO_CAMERA_FLASH_MODE_PIN//PIN: FLASH_STROBE, GPIO114
#define BD7710_TX_MASK   GPIO_CAMERA_FLASH_TX1_PIN//PIN: FLASH_TX, GPIO12
#define BD7710_RESETB    GPIO_CAMERA_FLASH_RESET_PIN//PIN: FLASH_RST, GPIO13
...
static int BD7710_chip_init(struct BD7710_chip_data *chip)
{
    ...
    mt_set_gpio_mode(BD7710_STRB0, 0);
    mt_set_gpio_dir(BD7710_STRB0, GPIO_DIR_OUT);
    mt_set_gpio_out(BD7710_STRB0, GPIO_OUT_ONE);

    mt_set_gpio_mode(BD7710_STRB1, 0);
    mt_set_gpio_dir(BD7710_STRB1, GPIO_DIR_OUT);
    mt_set_gpio_out(BD7710_STRB1, GPIO_OUT_ZERO);

    mt_set_gpio_mode(BD7710_TX_MASK, 0);
    mt_set_gpio_dir(BD7710_TX_MASK, GPIO_DIR_OUT);
    mt_set_gpio_out(BD7710_TX_MASK, GPIO_OUT_ZERO);
	mdelay(1);
    mt_set_gpio_mode(BD7710_RESETB, 0);
    mt_set_gpio_dir(BD7710_RESETB, GPIO_DIR_OUT);
    mt_set_gpio_out(BD7710_RESETB, GPIO_OUT_ONE);
	mdelay(5);
    mt_set_gpio_out(BD7710_RESETB, GPIO_OUT_ZERO);
	mdelay(5);
    mt_set_gpio_out(BD7710_RESETB, GPIO_OUT_ONE);
	mdelay(10);
	do {
...
int FL_Enable(void)
{
	...
	//Enable all leds (LED1~3)
	BD7710_read_reg(BD7710_i2c_client, 0x05, &reg_val);
	BD7710_write_reg(BD7710_i2c_client, 0x05, reg_val | 0x07);
	
	if(g_duty == 0)//torch
	{
		BD7710_write_reg(BD7710_i2c_client, 0x02, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x01, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x00, 0x22); //225ma torch output_en'
		udelay(50);
		mt_set_gpio_out(BD7710_STRB1, GPIO_OUT_ONE);
	...
	{
		u8 led2, led13;
		brightness = (g_duty * 50) + 250; //min=250mA step=50mA total_steps=15
		led2 = brightness / 100; //led2 provide half current
		led13 = (brightness - led2*50)/50; //led13 provide another half

		PK_DBG("FL_enable flash brightness=%d, led2=%d, led13=%d\n",
			    brightness,led2, led13);
		BD7710_write_reg(BD7710_i2c_client, 0x02, led13|0x80); 
		BD7710_write_reg(BD7710_i2c_client, 0x01, led2|0x80);
		udelay(50);
		mt_set_gpio_out(BD7710_STRB1, GPIO_OUT_ZERO);
...
int FL_Disable(void)
{
	...
	if((chip->mode == 2)||(chip->mode == 1))
	{
	    ...
	    mt_set_gpio_out(BD7710_RESETB, GPIO_OUT_ZERO);
		mdelay(1);
	    mt_set_gpio_out(BD7710_STRB0, GPIO_OUT_ZERO);
				  ...
				  enum led_brightness brightness)
{
	...
	else
	{
		...
		BD7710_write_reg(BD7710_i2c_client, 0x02, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x01, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x00, 0x22); //75ma torch output_en'
		udelay(50);
		mt_set_gpio_out(BD7710_STRB1, GPIO_OUT_ONE);
	}
	#elif defined(FLASH_RT9387)
...
static ssize_t torch_cntl_store(struct device *dev,
...
#ifndef CONTROL_SEPARATE	
	brightness *= 25;
	brightness = brightness > 525 ? 525 : brightness;
	
	led13 = brightness /50;
	if (led13 > 7) {
		led2 = (brightness - 350) / 25;
	} else {
		led2 = (brightness - led13 * 50) / 25;
	}
#endif

	led2 = led2 > 7 ? 7 : led2;
	led13 = led13 > 7 ? 7 : led13;

	reg_val = led2 | (led13 << 3);
	PK_DBG("%s: brightness=%d, led2=%d, led13=%d reg_val=%x\n",__func__, brightness,
		led2, led13, reg_val);

	BD7710_write_reg(BD7710_i2c_client, 0x00, reg_val); //75ma torch output_en'
	udelay(50);
	mt_set_gpio_out(BD7710_STRB1, GPIO_OUT_ONE);
...
static ssize_t torch_cntl_show(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
	return sprintf(buf, "delete to edit\n");
}


static struct device_attribute torch_cntl_attr =__ATTR(torch_cntl, (S_IRUGO | S_IWUGO),
                        torch_cntl_show,
                        torch_cntl_store);
...
static int flashchip_probe(struct platform_device *dev)
{
	...
	sysfs_create_file(&dev->dev.kobj, &torch_cntl_attr);
