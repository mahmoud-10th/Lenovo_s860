...
int BattVoltToTemp(int dwVolt)
{
	...
	/*lenovo-sw weiweij added for no battery power on*/	
	#ifdef LENOVO_CHARGING_STANDARD
          if (KAL_FALSE ==  lenovo_battery_is_battery_exist(TRes_temp, RBAT_PULL_DOWN_R))
		  	return -20;
	#endif
	/*lenovo-sw weiweij added for no battery power on end*/
...
#ifdef LENOVO_ZCV_TABLE_INIT
#define LENOVO_ZCV_PROFILE_SIZE 101
static BATTERY_PROFILE_STRUC lenovo_zcv_battery_profile_t0[LENOVO_ZCV_PROFILE_SIZE][LENOVO_ZCV_PROFILE_SIZE] = {0, 0};
static BATTERY_PROFILE_STRUC lenovo_zcv_battery_profile_t1[LENOVO_ZCV_PROFILE_SIZE][LENOVO_ZCV_PROFILE_SIZE] = {0, 0};
static BATTERY_PROFILE_STRUC lenovo_zcv_battery_profile_t2[LENOVO_ZCV_PROFILE_SIZE][LENOVO_ZCV_PROFILE_SIZE] = {0, 0};
static BATTERY_PROFILE_STRUC lenovo_zcv_battery_profile_t3[LENOVO_ZCV_PROFILE_SIZE][LENOVO_ZCV_PROFILE_SIZE] = {0, 0};
static BATTERY_PROFILE_STRUC lenovo_zcv_battery_profile_temperature[LENOVO_ZCV_PROFILE_SIZE][LENOVO_ZCV_PROFILE_SIZE] = {0, 0};

static int lenovo_zcv_set_battery_profile(kal_int32 temperature)
{
	BATTERY_PROFILE_STRUC_P profile_p, lenovo_profile_p;
	int i, j, index, saddles;
	int temp;
	int vol_step;

	bm_print(BM_LOG_CRTI, "%s temp=%d\r\n", __func__, temperature);		
	
	saddles = sizeof(battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);;	

	switch(temperature)
	{
		case TEMPERATURE_T0:
			lenovo_profile_p = &lenovo_zcv_battery_profile_t0[0];
			profile_p = &battery_profile_t0[0];
			break;
		case TEMPERATURE_T1:
			lenovo_profile_p = &lenovo_zcv_battery_profile_t1[0];
			profile_p = &battery_profile_t1[0];
			break;
		case TEMPERATURE_T2:
			lenovo_profile_p = &lenovo_zcv_battery_profile_t2[0];
			profile_p = &battery_profile_t2[0];
			break;
		case TEMPERATURE_T3:
			lenovo_profile_p = &lenovo_zcv_battery_profile_t3[0];
			profile_p = &battery_profile_t3[0];
			break;
		default:
        		bm_print(BM_LOG_CRTI, "%s invalid temp=%d\r\n", __func__, temperature);			
			return -1;
	}
	
	//lenovo_profile_p->voltage = profile_p->voltage;
	//lenovo_profile_p->percentage = profile_p->percentage;
	index = 0;	
	for (i = 0; i < saddles; i++)
	{		
		temp = (profile_p + i + 1)->percentage - (profile_p + i )->percentage;
		if(temp==0)
		{
			if((profile_p + i )->percentage==100)
			{
				(lenovo_profile_p + index)->voltage = (profile_p + i)->voltage;
				(lenovo_profile_p + index)->percentage = (profile_p + i)->percentage;
				break;				
			}else
				continue;
		}else
		{
			vol_step = ((profile_p + i)->voltage - (profile_p + i + 1)->voltage)/temp;
			for(j=0;j<temp;j++)
			{
				(lenovo_profile_p + index)->voltage = (profile_p + i)->voltage - vol_step*j;
				(lenovo_profile_p + index)->percentage = (profile_p + i)->percentage + j;
				index++;
			}
		}

		if(index>=LENOVO_ZCV_PROFILE_SIZE)
			break;
	}
	...
	if(0)
	{
		for(i=0;i<LENOVO_ZCV_PROFILE_SIZE;i++)
	        	bm_print(BM_LOG_CRTI, "%d (%d,%d)\r\n", i, (lenovo_profile_p+i)->voltage, (lenovo_profile_p+i)->percentage);			
	}
	
	return 0;	
}

static int lenovo_zcv_init_battery_profile(void)
{
      	bm_print(BM_LOG_CRTI, "%s\r\n", __func__);			
	
	lenovo_zcv_set_battery_profile(TEMPERATURE_T0);
	lenovo_zcv_set_battery_profile(TEMPERATURE_T1);
	lenovo_zcv_set_battery_profile(TEMPERATURE_T2);
	lenovo_zcv_set_battery_profile(TEMPERATURE_T3);
	
	return 0;	
}
static BATTERY_PROFILE_STRUC_P lenovo_zcv_get_battery_profile(kal_uint32 temperature)
{
	switch (temperature)
	{
		case TEMPERATURE_T0:
			return &lenovo_zcv_battery_profile_t0[0];
			break;    
		case TEMPERATURE_T1:
			return &lenovo_zcv_battery_profile_t1[0];
			break;
		case TEMPERATURE_T2:
			return &lenovo_zcv_battery_profile_t2[0];
			break;
		case TEMPERATURE_T3:
			return &lenovo_zcv_battery_profile_t3[0];
			break;
		case TEMPERATURE_T:
			return &lenovo_zcv_battery_profile_temperature[0];
			break;
		default:
			return NULL;
			break;
...
{
#ifdef LENOVO_ZCV_TABLE_INIT
	return LENOVO_ZCV_PROFILE_SIZE;
#else
...
{
#ifdef LENOVO_ZCV_TABLE_INIT
	return lenovo_zcv_get_battery_profile(temperature);
#else
    switch (temperature)
    {
        case TEMPERATURE_T0:
...
#ifdef LENOVO_ZCV_TABLE_INIT	
	if((temperature-low_temperature)>(high_temperature-low_temperature)/2)
        	(temp_profile_p + i)->percentage = (high_profile_p + i)->percentage;
	else
        	(temp_profile_p + i)->percentage = (low_profile_p + i)->percentage;
#else
        (temp_profile_p + i)->percentage = (high_profile_p + i)->percentage;
#endif
...
void dod_init(void)
{
    ...
    {
/*lenovo-sw weiweij remove the operation of charger detetc*/
#if 1
        if(((g_rtc_fg_soc != 0) && ((g_rtc_fg_soc-gFG_capacity_by_v) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE)))
#else
...
#endif
/*lenovo-sw weiweij remove the operation of charger detetc end*/
    ...
    {
/*lenovo-sw weiweij modified for power on soc det*/
#if 1    
    	if(((g_rtc_fg_soc > 1)	
#else
    	if(((g_rtc_fg_soc != 0)	
#endif
/*lenovo-sw weiweij modified for power on soc det end*/
		&& ((gFG_capacity_by_v-g_rtc_fg_soc) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT || g_boot_reason == BR_TOOL_BY_PASS_PWK || g_boot_reason == BR_2SEC_REBOOT || g_boot_mode == RECOVERY_BOOT)))
        {
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }
...
/*lenovo-sw weiweij added 20130723*/
#if 1
extern kal_bool upmu_is_chr_det(void);
extern void lenovo_battery_disable_charging(void);
extern void lenovo_battery_enable_charging(void);
#endif
/*lenovo-sw weiweij added 20130723 end*/
...
/*lenovo-sw weiweij modified for using isense as bat voltage*/	
#ifdef LENOVO_USING_ISENSE_AS_VBAT
    //ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_I_SENSE, &vol_bat);
	if(bat_is_charger_exist() == KAL_TRUE)
	{
		int temp = 5;

		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_I_SENSE, &temp);
		lenovo_battery_disable_charging();
		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_I_SENSE, &vol_bat);
		lenovo_battery_enable_charging();
		bm_print(BM_LOG_CRTI, "[oam_init_inf] ww_debug vol=%d, temp=%d\n", vol_bat, temp);
	}else
		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_I_SENSE, &vol_bat);
#else
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_SENSE, &vol_bat);
#endif
/*lenovo-sw weiweij modified for using isense as bat voltage end*/
    ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_OCV, &gFG_voltage);
...
/*lenovo-sw weiweij modified for using isense as bat voltage end*/	
    ...
    bm_print(BM_LOG_CRTI, "[oam_run] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", 
    ...
    bm_print(BM_LOG_CRTI, "[oam_total] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
...
#ifdef LENOVO_ZCV_TABLE_INIT
	lenovo_zcv_init_battery_profile();
#endif
