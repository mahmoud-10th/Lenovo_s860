...
/*lenovo-sw weiweij added for charging sleep*/
#ifdef LENOVO_CHARGING_STANDARD
//#define LENOVO_CHARGING_SLEEP 1
#if (LENOVO_CHARGING_SLEEP)
#define SPM_WAKE_PERIOD 				600 //must as same as the same var in mt_spm_sleep.c
const int LENOVO_CHARGInG_SLEEP_PERIOD	= 10;
int lenoo_charng_sleep_period = SPM_WAKE_PERIOD;
#endif
#endif
/*lenovo-sw weiweij added for charging sleep end*/

/* lenovo.sw chenyb1 begin. Add for keep poweroff alarm ringing when battery low */
#ifdef LENOVO_LOWBAT_PWROFF_ALARM
#define BATLOW_RTC_ALARM_THRESHOLD	   3250
#define BATLOW_RTC_ALARM_FAKE_TIMES		15
#define BATLOW_RTC_ALARM_SHUTDOWN_SOC   1
#define BATLOW_RTC_ALARM_FAKE_SOC   (BATLOW_RTC_ALARM_SHUTDOWN_SOC+1)
#endif
/* lenovo.sw chenyb1 end. Add for keep poweroff alarm ringing when battery low */

/*Begin lenovo-sw wengjun1 add for control glove function. 2013-5-7*/
#if defined(LENOVO_PROJECT_AUX) || defined(LENOVO_CTP_GLOVE_CONTROL) || defined(LENOVO_INTERFERNCE_PROTECT)
static cable_in_flag = 1;
static first_cable_in_set_flag = 0;
#if defined(LENOVO_PROJECT_AUX) || defined(LENOVO_INTERFERNCE_PROTECT)
    int set_tp_protect(bool flag);
#else
    int get_tpd_glove_pre_status(void);//wengjun1 add for record glove function status before plug in.
...
/* lenovo.sw chenyb1 begin. Add for keep poweroff alarm ringing when battery low */
#ifdef LENOVO_LOWBAT_PWROFF_ALARM
	static kal_int32 rtc_alarm_fake_counter = 0;
	battery_xlog_printk(BAT_LOG_CRTI, "before rtc_alrm_fake BAT_CAPACITY = %d, g_boot_reason=%d, vol=%d, count=%d\n",
		bat_data->BAT_CAPACITY, g_boot_reason, bat_data->BAT_InstatVolt, rtc_alarm_fake_counter);
	if ((g_boot_reason == BR_RTC) && (bat_data->BAT_CAPACITY <= BATLOW_RTC_ALARM_SHUTDOWN_SOC) && (bat_data->BAT_InstatVolt >=BATLOW_RTC_ALARM_THRESHOLD))
	{
		if (rtc_alarm_fake_counter <= BATLOW_RTC_ALARM_FAKE_TIMES)
		{
			// fake the soc for keeping poweroff alarm ringing
			bat_data->BAT_CAPACITY = BATLOW_RTC_ALARM_FAKE_SOC;
			rtc_alarm_fake_counter++;
			battery_xlog_printk(BAT_LOG_CRTI, "fake the soc for keeping poweroff alarm ringing BAT_CAPACITY = %d, count=%d\n",bat_data->BAT_CAPACITY, rtc_alarm_fake_counter);
		}
	}
#endif //LENOVO_LOWBAT_PWROFF_ALARM
/* lenovo.sw chenyb1 end. Add for keep poweroff alarm ringing when battery low */
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
    ...
    {
/*lenovo-sw weiweij added for ui_soc-- in charging mode*/    
    	if(BMT_status.charger_exist == KAL_TRUE)
    	{
    		    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] mt_battery_0Percent_tracking_check(), BMT_status.charger_exist return\r\n");   
			resetBatteryMeter = KAL_FALSE;
			return resetBatteryMeter;
    	}
/*lenovo-sw weiweij added for ui_soc-- in charging mode*/    
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
    ...
    {   
		...
		{
/*lenovo-sw weiweij added for ui_soc sync in charging*/
#if 1
...
/*lenovo-sw weiweij added for low vol power off check */
#ifdef LENOVO_LOW_POWER_SYSTEMOFF_DET
static kal_bool lenovo_system_off_det(void)
{
	int bat_vol;
	...
	if(bat_vol <= SYSTEM_OFF_VOLTAGE)
	{
		counter++;
	}else
		counter = 0;
	
    battery_xlog_printk(BAT_LOG_CRTI,"ww_debug 1 counter=%d, vol=%d soc=%d\n", counter, bat_vol, BMT_status.UI_SOC);
	if(counter>=2)
	{
		counter = 0;
		BMT_status.UI_SOC--;
    ...
    battery_xlog_printk(BAT_LOG_CRTI,"ww_debug 2 vol=%d soc=%d\n", bat_vol, BMT_status.UI_SOC);

	return resetBatteryMeter;
}
#endif
/*lenovo-sw weiweij added for low vol power off check end*/
...
static void battery_update(struct battery_data *bat_data)
{
	...
	bat_data->BAT_PRESENT = BMT_status.bat_exist;
			...
			battery_xlog_printk(BAT_LOG_CRTI, "[ww_debug] BMT_status.charger_exist=%d, BMT_status.bat_charging_state=%d\r\n", BMT_status.charger_exist, BMT_status.bat_charging_state);
...
/*lenovo-sw weiweij added for low vol power off check */
#ifdef LENOVO_LOW_POWER_SYSTEMOFF_DET
		{
			...
			resetBatteryMeter =lenovo_system_off_det();
		}
#else			
			resetBatteryMeter = mt_battery_nPercent_tracking_check();
#endif
...
/*lenovo-sw weiweij added for battery temp update in charging sleep stage*/
#if defined( LENOVO_BATTERY_TEMP_AVERAGE)&&defined(LENOVO_CHARGING_SLEEP)	
static int lenovo_battery_temp_average_method_init(kal_int32 *bufferdata, kal_int32 *sum, kal_int32 *lenovo_sum)
{
	kal_uint32 i;
	kal_int32 init_data;
	static kal_bool batteryBufferFirst = KAL_TRUE;
	...
	if(batteryBufferFirst == KAL_TRUE)
	{
		init_data = *sum/BATTERY_AVERAGE_SIZE;
		
		battery_xlog_printk(BAT_LOG_CRTI, "data= (%d) \n", init_data);
		
		for (i=0; i<LENOVO_BATTERY_TEMP_AVERAGE_SIZE; i++)
		{
			bufferdata[i] = init_data;            
		}
		*lenovo_sum = init_data * LENOVO_BATTERY_TEMP_AVERAGE_SIZE;
	}
...
static kal_uint32 lenovo_battery_temp_average_method(kal_int32 *org_buf, kal_uint32 data, kal_int32 *sum, kal_uint8 batteryIndex)
{
	kal_uint32 avgdata;
	static kal_int32 batteryTempBuffer[LENOVO_BATTERY_TEMP_AVERAGE_SIZE];
	static kal_int32 temperature_sum = 0;
	 kal_uint8 index;

//	avgdata = mt_battery_average_method(org_buf, data, sum, batteryIndex);
	//function mt_battery_average_method
	mt_battery_average_method_init(org_buf, data, sum);
	lenovo_battery_temp_average_method_init(batteryTempBuffer, sum, &temperature_sum);

	*sum -=	org_buf[batteryIndex];
	*sum +=  data;
	org_buf[batteryIndex] = data;
    	avgdata = (*sum)/BATTERY_AVERAGE_SIZE;
	//function mt_battery_average_method end
	
	index = batteryIndex%LENOVO_BATTERY_TEMP_AVERAGE_SIZE;

	temperature_sum -= batteryTempBuffer[index];
	temperature_sum +=  data;
	batteryTempBuffer[index] = data;

	if(lenoo_charng_sleep_period == LENOVO_CHARGInG_SLEEP_PERIOD)
	{
		avgdata = (temperature_sum)/LENOVO_BATTERY_TEMP_AVERAGE_SIZE;
		battery_xlog_printk(BAT_LOG_CRTI, "lenovo temp bufferdata[%d]= (%d) avgdata=%d sum=%d\n", index,batteryTempBuffer[index], avgdata, temperature_sum);
	}else
		battery_xlog_printk(BAT_LOG_CRTI, "temp bufferdata[%d]= (%d) avgdata=%d sum=%d\n", batteryIndex,org_buf[batteryIndex], avgdata, sum);

	return avgdata;
}
#endif
/*lenovo-sw weiweij added for battery temp update in charging sleep stage end*/
...
/*lenovo-sw weiweij added for battery temp update in charging sleep stage*/
#if defined( LENOVO_BATTERY_TEMP_AVERAGE)&&defined(LENOVO_CHARGING_SLEEP)	
	BMT_status.temperature = lenovo_battery_temp_average_method(&batteryTempBuffer[0],temperature, &temperature_sum, batteryIndex);
#else
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if( BMT_status.charger_exist == KAL_TRUE)
    {
        ...
        #endif        
/*lenovo-sw weiweij modified for high voltage charger protext voltage*/
#ifdef LENOVO_H_CHARGER
	if(BMT_status.charger_vol > LENOVO_V_CHARGER_MAX)
#else
        if ( BMT_status.charger_vol >= V_CHARGER_MAX )
#endif		
/*lenovo-sw weiweij modified for high voltage charger protext voltage end*/
        {
...
{
   /*Lenovo-sw begin yexh1 add 2013-7-16,add for lenovo charging*/ 
    #ifndef LENOVO_CHARGING_STANDARD
    ...
    #endif	
    /*Lenovo-sw end yexh1 add 2013-7-16,add for lenovo charging*/ 
...
/*lenovo-sw weiweij modified for ad tolarence*/
#if 0
    if(BMT_status.bat_vol > 4350)
#else
#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
/*lenovo-sw weiweij modified for changing bat vol warning threshold to 4480 for IC bq24196*/
	#if defined(MTK_BQ24196_SUPPORT)
	    if(BMT_status.bat_vol > 4480)
	#else
	    if(BMT_status.bat_vol > 4400)
	#endif
/*lenovo-sw weiweij modified for changing bat vol warning threshold to 4480 for IC bq24196 end*/
#else
    if(BMT_status.bat_vol > 4350)
#endif		
#endif
/*lenovo-sw weiweij modified for ad tolarence end*/
...
/*lenovo-sw weiweij modified for high voltage charger protext voltage warnging*/
#ifdef LENOVO_H_CHARGER
	if(BMT_status.charger_vol > LENOVO_V_CHARGER_MAX)
    {
        g_BatteryNotifyCode |= 0x0001;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BMT_status.charger_vol(%ld) > %d mV\n", BMT_status.charger_vol, LENOVO_V_CHARGER_MAX);
    }
#else
	if(BMT_status.charger_vol > V_CHARGER_MAX)
    {
...
#endif
/*lenovo-sw weiweij modified for high voltage charger protext voltage warning end*/
    else
    {
        g_BatteryNotifyCode &= ~(0x0001);
    }
...
/*lenovo-sw weiweij added for charging sleep*/
		...
		if(BMT_status.charger_type==STANDARD_CHARGER){		
			...
			lenoo_charng_sleep_period = LENOVO_CHARGInG_SLEEP_PERIOD;
		}else
...
#if defined(LENOVO_PROJECT_AUX) || defined(LENOVO_CTP_GLOVE_CONTROL) || defined(LENOVO_INTERFERNCE_PROTECT)
        cable_in_flag = 1;
        if(first_cable_in_set_flag == 0)
        {
            #if defined(LENOVO_PROJECT_AUX) || defined(LENOVO_INTERFERNCE_PROTECT)
            printk("[wj]set flag is true.\n");
            set_tp_protect(TRUE);
            #else
            printk("[wj]set glove function disable.\n");
            set_glove_func(0);
            #endif
        }
#endif
...
/*lenovo-sw weiweij added for charging sleep*/
#if (LENOVO_CHARGING_SLEEP)
	lenoo_charng_sleep_period = SPM_WAKE_PERIOD;
#endif	  
/*lenovo-sw weiweij added for charging sleep end*/
...
/*Begin lenovo-sw wengjun1 add for control glove function. 2013-5-7*/
        ...
        if(cable_in_flag == 1)
        {
            ...
            #else
            printk("[wj]pre_status=%d.\n", get_tpd_glove_pre_status());
            if(get_tpd_glove_pre_status() == 1)
            {
                printk("[wj]set glove function enable.\n");
                set_glove_func(1);
            }
            #endif
            cable_in_flag = 0;
            first_cable_in_set_flag = 0;
        }
#endif
/*End lenovo-sw wengjun1 add for control glove function. 2013-5-7*/
        mt_usb_disconnect();          
...
/*lenovo-sw weiweij added for h_vol_charger*/
#ifdef LENOVO_H_CHARGER
	 	lenovo_h_charger_prepare();
#endif
/*lenovo-sw weiweij added for h_vol_charger end*/
        }
        else
        {
...
/*lenovo-sw weiweij added for bat_cali_status sync*/
#ifdef LENOVO_CHARGING_STANDARD
		lenovo_battery_set_Qmax_cali_status(0);
#endif
/*lenovo-sw weiweij added for bat_cali_status sync end*/
...
/*lenovo-sw weiweij added for charging sleep*/
		...
		{
			wake_lock(&battery_suspend_lock);	
			battery_xlog_printk(BAT_LOG_CRTI, "ww_debug lock.\n");    
		}
#endif
#endif
/*lenovo-sw weiweij added for charging sleep end*/	
...
/*lenovo-sw weiweij added for charging sleep*/
		...
		if(lenoo_charng_sleep_period==LENOVO_CHARGInG_SLEEP_PERIOD)
		{
			...
			battery_xlog_printk(BAT_LOG_CRTI, "ww_debug unlock.\n");    
			wake_lock(&battery_suspend_lock);				
		}
#endif
#endif
...
/*lenovo-sw weiweij modified for high voltage charger protext voltage*/
#ifdef LENOVO_H_CHARGER	
	kal_uint32 hv_voltage = BATTERY_VOLT_10_000000_V;
#else
