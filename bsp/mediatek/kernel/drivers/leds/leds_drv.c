...
#include "leds_drv.h"
#include "lcm_drv.h"//lenovo.sw2 houdz1 20140124 add 
...
extern LCM_PARAMS *lcm_params;//lenovo.sw2 houdz1 add
...
static void mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	///lenovo add begin by houdz1@lenovo.com
	#if  defined(LENOVO_BACKLIGHT_LIMIT)
		#if  defined(MTK_AAL_SUPPORT)
		int temp_level;
		temp_level = level;
		if(temp_level >> LED_RESERVEBIT_SHIFT)
		{
			if(LED_RESERVEBIT_PATTERN != (temp_level >> LED_RESERVEBIT_SHIFT))
			{
				//sanity check for hidden code
				printk("incorrect input : %d,%d\n" , temp_level , (temp_level >> LED_RESERVEBIT_SHIFT));
				//return;
			}
			...
			temp_level &= ((1 << LED_RESERVEBIT_SHIFT) - 1);
		
			if((temp_level +1) > (1 << 10))
			{
				//clip to max value
				temp_level = (1 << 10) - 1;
			}
		
			level = (temp_level >> (10 - 8));//brightness is 8 bit level
			if(level > 255)
			{
				level = 255;
			}
		...
		#endif
		if((level>0)&&(lcm_params!=NULL)&&(strcmp(led_data->cust.name,"lcd-backlight") == 0)) {
		int bl_app_min;
		int bl_app_def;
		int bl_app_max;
		int bl_bsp_min;
		...
		int bl_bsp_max;
		int temp=level;
		int low_region_level;
		int high_region_level;

		bl_app_min = (lcm_params->bl_app.min==0)?1:(lcm_params->bl_app.min);
		bl_app_def = (lcm_params->bl_app.def==0)?102:(lcm_params->bl_app.def);
		bl_app_max = (lcm_params->bl_app.max==0)?255:(lcm_params->bl_app.max);
		bl_bsp_min = (lcm_params->bl_bsp.min==0)?5:(lcm_params->bl_bsp.min);
		bl_bsp_def = (lcm_params->bl_bsp.def==0)?102:(lcm_params->bl_bsp.def);
		bl_bsp_max = (lcm_params->bl_bsp.max==0)?255:(lcm_params->bl_bsp.max);
		low_region_level = (((level-bl_app_min)*(bl_bsp_def-bl_bsp_min))/(bl_app_def-bl_app_min)+bl_bsp_min);
		high_region_level = (((level-bl_app_def)*(bl_bsp_max-bl_bsp_def))/(bl_app_max-bl_app_def)+bl_bsp_def);
		//we must confirm the bl_app value was set correct in lcm drivers
		if((lcm_params->bl_app.def==APP_BL_DEFAULT)){
			temp = (level<bl_app_def)?low_region_level:high_region_level;
			printk("[JX] %s level=%d temp=%d\n",__func__,level, temp);
			level = temp;
		}
		#if defined(MTK_AAL_SUPPORT)
			temp_level = ((((1 << 10) - 1)*level + 127)/255);
			temp_level |= (1<<16);
			level = temp_level;
			printk("[JX1] %s level=%d temp=%d\n",__func__,level, temp);
		#endif
		
	}
	#endif
	///lenovo add end by houdz1@lenovo.com
