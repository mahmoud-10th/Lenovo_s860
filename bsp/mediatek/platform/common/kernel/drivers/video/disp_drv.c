...
DISP_STATUS DISP_SetCabcMode(unsigned int mode)
{
...
DISP_STATUS DISP_SetInverseMode(unsigned int mode)
{
	DISP_STATUS ret = DISP_STATUS_OK;
...
extern lenovo_disp_feature_info_t disp_feature_info[MTKFB_MAX_DISPLAY_COUNT];//lenovo add by jixu@lenovo.com
extern lenovo_disp_feature_state_t disp_feature_state[MTKFB_MAX_DISPLAY_COUNT];//lenovo add by jixu@lenovo.com
...
#if 1//lenovo jixu modify
    if((!is_ipoh_bootup) || (is_ipoh_bootup && (lcm_params->type==LCM_TYPE_DSI && lcm_params->dsi.mode == CMD_MODE)) ||lcm_params->dsi.mode == SYNC_PULSE_VDO_MODE || lcm_params->dsi.mode == BURST_VDO_MODE)
#else
