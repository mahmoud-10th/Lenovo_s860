...
char mtkfb_lcm_version[256] = {0};//lenovo jixu add
...
static int lcm_info_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    p += sprintf(p,"name=%s; width=%d; height=%d; mode=%s; lane=%d; bl_app:%d,%d,%d; bl_bsp:%d,%d,%d;\n",
			lcm_drv->name,
			lcm_params->width,
			lcm_params->height,
			(lcm_params->dsi.mode==0)?"CMD":"VDO",
			lcm_params->dsi.LANE_NUM,
			lcm_params->bl_app.min,
			lcm_params->bl_app.def,
			lcm_params->bl_app.max,
			lcm_params->bl_bsp.min,
			lcm_params->bl_bsp.def,
			lcm_params->bl_bsp.max);
