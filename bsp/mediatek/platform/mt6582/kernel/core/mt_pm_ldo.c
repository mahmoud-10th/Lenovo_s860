...
#if 1
bool hwCameraPowerOn(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
    UINT32 i = 0;
    int j=0, k=0;
...
bool hwCameraPowerDown(MT65XX_POWER powerId, char *mode_name)
{
    UINT32 i;
#if 1	
    BOOL bFind = FALSE;    
...
bool cameraPowerOnOff(MT65XX_POWER powerId,bool on)
	{
    pmic_ldo_enable(powerId, on);
	return TRUE;
	}

EXPORT_SYMBOL(hwCameraPowerOn);
EXPORT_SYMBOL(hwCameraPowerDown);
EXPORT_SYMBOL(cameraPowerOnOff);
#endif
