...
#if 1
	void (*eint_threaded_func[EINT_MAX_CHANNEL])(void);
#endif
/*Lenovo-sw add end*/
...
void mt_eint_registration_threaded(unsigned int eint_num, unsigned int pol, void(EINT_FUNC_PTR)(void), void(threaed_isr)(void), unsigned int is_auto_umask)
{
	if (!EINT_FUNC_PTR && !threaed_isr) {
		WARN(1, "Either ISR or Threaded ISR needed!!!");
		return ;
	}
	if (eint_num < EINT_MAX_CHANNEL) {
		mt_eint_mask(eint_num);
	...
	mt_eint_registration(eint_num, pol, EINT_FUNC_PTR, is_auto_umask);
}

static irqreturn_t mt_eint_threaded_isr(int irq, void *dev_id)
{
	int i;
	//printk("%s thread_irq=%d\n", __func__, irq);
	for (i=0; i<EINT_MAX_CHANNEL; i++){
		if (EINT_FUNC.eint_threaded_func[i]) {
			EINT_FUNC.eint_threaded_func[i]();
		}
	}
	return IRQ_HANDLED;
}
    ...
    for (reg_base = 0; reg_base < EINT_MAX_CHANNEL; reg_base+=32) {
        ...
        if (status_check) {
            ...
            if ((EINT_FUNC.is_deb_en[index] == 1) &&
				...
				/*Lenovo-sw chenglong1 add for threaded isr*/
			       #if 1
				if (EINT_FUNC.eint_threaded_func[index]) {
					need_wake_thread = 1;
					//printk("%s irq: %d\n", __func__, index);
				}
				#endif
...
/*Lenovo-sw chenglong1 add for threaded isr*/
#if 1
	EINT_FUNC.eint_threaded_func[i] = NULL;
#endif
/*Lenovo-sw add end*/
...
#else
	if (request_threaded_irq(EINT_IRQ, mt_eint_isr, mt_eint_threaded_isr, IRQF_TRIGGER_HIGH, "EINT", NULL)) {
		printk(KERN_ERR "EINT IRQ LINE NOT AVAILABLE!!\n");
	}
#endif
/*Lenovo-sw modify end*/
...
/*Lenovo-sw chenglong1 add for threaded isr*/
#if 1
EXPORT_SYMBOL(mt_eint_registration_threaded);
#endif
/*Lenovo-sw add end*/
