...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
#ifdef LENOVO_COMPATIBLE_DVT_EARPHONE
    mt_eint_mask(CUST_EINT_ACCDET_EXT_NUM);
#endif
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		/* lenovo-sw zhouwl, 2013-10-12, for FSA8049 */
		#if defined(LENOVO_USE_HPTPA6136_CODEC)
		mt_set_gpio_dir(GPIO_HPTPA6136_HI_Z_PIN, GPIO_DIR_OUT); //Hi_Z
		mt_set_gpio_out(GPIO_HPTPA6136_HI_Z_PIN, GPIO_OUT_ONE);
		mt_set_gpio_dir(GPIO_HPTPA6136_GAIN_PIN, GPIO_DIR_OUT); //Gain
		mt_set_gpio_out(GPIO_HPTPA6136_GAIN_PIN, GPIO_OUT_ZERO);
		mt_set_gpio_dir(GPIO_HPTPA6136_SDB_PIN, GPIO_DIR_OUT); //SDB
		mt_set_gpio_out(GPIO_HPTPA6136_SDB_PIN, GPIO_OUT_ONE);
		#endif
		/* lenovo-sw zhouwl, 2013-10-12, for FSA8049 */
...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
#ifdef LENOVO_COMPATIBLE_DVT_EARPHONE
    mt_eint_unmask(CUST_EINT_ACCDET_EXT_NUM);	
#endif
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
...
/* LENOVO.SW Begin.chenyb1 2013.1.14.Fix plug out with button down error event */
              ...
              mt_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, 10);
#endif
/* LENOVO.SW End.chenyb1 2013.1.14.Fix plug out with button down error event */
	}
...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
#ifdef LENOVO_COMPATIBLE_DVT_EARPHONE
static void accdet_ext_eint_func(void)
{
	int ret=0;
	if(cur_eint_state ==  EINT_PIN_PLUG_IN ) 
	{
	...
	*/
		if (CUST_EINT_ACCDET_TYPE == CUST_EINTF_TRIGGER_HIGH){
					mt_eint_set_polarity(CUST_EINT_ACCDET_EXT_NUM, (1));
		}else{
					mt_eint_set_polarity(CUST_EINT_ACCDET_EXT_NUM, (0));
		}

#ifdef ACCDET_SHORT_PLUGOUT_DEBOUNCE
        mt_eint_set_hw_debounce(CUST_EINT_ACCDET_EXT_NUM, CUST_EINT_ACCDET_EXT_DEBOUNCE_CN);
#endif
...
/* LENOVO.SW End.chenyb1 2013.1.14.Fix plug out with button down error event */
	...
	*/
	...
	/* update the eint status */
#ifdef ACCDET_SHORT_PLUGOUT_DEBOUNCE
        mt_eint_set_hw_debounce(CUST_EINT_ACCDET_EXT_NUM, ACCDET_SHORT_PLUGOUT_DEBOUNCE_CN);
#endif        
		cur_eint_state = EINT_PIN_PLUG_IN;
...
#ifdef LENOVO_STD_LINECTL_EARPHONE
              // then is plug-out detect debounce
              mt_eint_set_hw_debounce(CUST_EINT_ACCDET_EXT_NUM, 10);
#endif
/* LENOVO.SW End.chenyb1 2013.1.14.Fix plug out with button down error event */
	}
...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
#ifdef LENOVO_COMPATIBLE_DVT_EARPHONE
	mt_set_gpio_mode(GPIO_ACCDET_EXT_EINT_PIN, GPIO_ACCDET_EXT_EINT_PIN_M_EINT);
	mt_set_gpio_dir(GPIO_ACCDET_EXT_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_ACCDET_EXT_EINT_PIN, GPIO_PULL_DISABLE); //To disable GPIO PULL.

	mt_eint_set_hw_debounce(CUST_EINT_ACCDET_EXT_NUM, CUST_EINT_ACCDET_EXT_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_ACCDET_EXT_NUM, CUST_EINT_ACCDET_EXT_TYPE, accdet_ext_eint_func, 0);
	ACCDET_DEBUG("[Accdet]accdet set EINT finished, accdet_eint_num=%d, accdet_eint_debounce_en=%d, accdet_eint_polarity=%d\n", CUST_EINT_ACCDET_EXT_NUM, CUST_EINT_ACCDET_EXT_DEBOUNCE_EN, CUST_EINT_ACCDET_EXT_TYPE);
