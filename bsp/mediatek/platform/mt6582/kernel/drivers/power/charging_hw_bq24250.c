 ...
 kal_uint32 charging_voltage_to_value(kal_uint32 val)
{
	...
	reg_val = (val - 3500)/20 + 1;
   	...
   	//battery_xlog_printk(BAT_LOG_CRTI, "ww_debug reg=0x%x \r\n", reg_val);
	//TODO: ASSERT(0);	// not find the value
	return reg_val;
}
...
/*const kal_uint32 LENOVO_CHARGING_TERM_ITERM[]=
{
	CHARGE_CURRENT_50_00_MA,	CHARGE_CURRENT_75_00_MA,	CHARGE_CURRENT_100_00_MA, 
	CHARGE_CURRENT_125_00_MA, 	CHARGE_CURRENT_150_00_MA, 	CHARGE_CURRENT_175_00_MA, 
	CHARGE_CURRENT_200_00_MA, 	CHARGE_CURRENT_225_00_MA, 	CHARGE_CURRENT_MAX
}; */
...
/*static kal_uint32 lenovo_charging_set_iterm(kal_uint32 val)
{
 	kal_uint32 status = STATUS_OK;
	kal_uint32 set_iterm;
	kal_uint32 array_size;
	kal_uint32 register_value;

	if(val >= CHARGE_CURRENT_200_00_MA)
	{
		val = CHARGE_CURRENT_200_00_MA;
	}

	array_size = GETARRAYNUM(LENOVO_CHARGING_TERM_ITERM);
	set_iterm = bmt_find_closest_level(LENOVO_CHARGING_TERM_ITERM, array_size, val);
	register_value = charging_parameter_to_value(LENOVO_CHARGING_TERM_ITERM, array_size ,set_iterm);
	bq24250_set_iterm(register_value);
...
void lenovo_charging_term_set_stage_2(void)
{
	if(lenovo_charging_stage==1)
	{
		...
		//lenovo_charging_set_iterm(CHARGE_CURRENT_150_00_MA);
		bq24250_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);
	}
}
...
void lenovo_charging_term_set_iterm(void)
{
	if((lenovo_charging_stage==1)&&(lenovo_charging_charger_type==STANDARD_CHARGER))
		bq24250_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_1);//lenovo_charging_set_iterm(CHARGE_CURRENT_200_00_MA);
	else
		bq24250_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);//lenovo_charging_set_iterm(CHARGE_CURRENT_150_00_MA);
}
#endif
