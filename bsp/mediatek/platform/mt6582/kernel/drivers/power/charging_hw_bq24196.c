...
const kal_uint32 INPUT_CS_VTH[]=
{
	...
	CHARGE_CURRENT_1200_00_MA, 	CHARGE_CURRENT_1500_00_MA, 	 CHARGE_CURRENT_2000_00_MA,  CHARGE_CURRENT_3000_00_MA
}; 
...
extern int lenovo_is_h_charger;
...
void lenovo_charging_term_set_stage_2(void)
{
	if(lenovo_charging_stage==1)
	{
		...
		bq24196_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);
...
{
//	printk("[LENOVO_CHARGING_TERM] %d, %d\n", lenovo_charging_stage, lenovo_charging_charger_type);
	
	if((lenovo_charging_stage==1)&&(lenovo_charging_charger_type==STANDARD_CHARGER))
	{
		bq24196_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_1);
	}else
		bq24196_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);
}
#endif
...
/*lenovo-sw weiweij added for control bq24196 by cpu, not 27531*/
#ifdef MTK_BQ27531_SUPPORT
	//bq27531_ctrl_ctrlenablecharge();
#endif
/*lenovo-sw weiweij added for control bq24196 by cpu , not 27531 end*/
...
/*lenovo-sw weiweij added for specail battery temperature protect*/
#ifdef LENOVO_SPECAIL_BATTERY_TEMP_PROTECT
	{
		int vol_val;
		int reg_val;
		vol_val = lenovo_specail_battery_temp_protest_set_cv_vol();
		reg_val = charging_voltage_to_value(vol_val);
		//printk("ww_debug: vol=%d, reg=0x%x\n", vol_val, reg_val);
		bq24196_set_vreg(reg_val);
	}
#else
	#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
        //fan5405_config_interface_liao(0x06,0x77); // ISAFE = 1250mA, VSAFE = 4.34V
		//bq24196_set_vindpm(0x6);
		bq24196_set_vreg(0x35);
    #else
        //fan5405_config_interface_liao(0x06,0x70);
		//bq24196_set_vindpm();
		bq24196_set_vreg(0x2c);
	#endif
#endif
/*lenovo-sw weiweij added for specail battery temperature protect end*/
	...
	bq24196_set_term_stat(0);
/*lenovo-sw weiweij modified for changing input current to 2A*/	
	bq24196_set_iinlim(0x06);
/*lenovo-sw weiweij modified for changing input current to 2A end*/
	bq24196_set_vindpm(0x08);
	bq24196_set_en_hiz(0);
	bq24196_set_en_term(1);
/*lenovo-sw weiweij added for charging terminate as 0.1c*/
#ifdef LENOVO_CHARGING_TERM
...
/*lenovo-sw weiweij added for charging terminate as 0.1c end*/
	...
	bq24196_set_res_ir(3);
	bq24196_set_vol_ir(7);

	bq24196_set_treg(3);
	bq24196_set_boost_lim(0);	
 ...
 static kal_uint32 charging_enable(void *data)
 {
	...
	if(KAL_TRUE == enable)
	{
/*lenovo-sw weiweij added for enable bq24196 by 27531*/
#ifdef MTK_BQ27531_SUPPORT
//	bq27531_ctrl_enablecharge();
#endif
/*lenovo-sw weiweij added for enable bq24196 by 27531 end*/
...
/*lenovo-sw weiweij added for enable bq24196 by 27531 end*/
 ...
 static kal_uint32 charging_set_input_current(void *data)
 {
    ...
    if(*(kal_uint32 *)data > CHARGE_CURRENT_3000_00_MA)
    {
