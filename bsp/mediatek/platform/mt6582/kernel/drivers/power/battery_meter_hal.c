...
/*lenovo-sw weiweij added for lenovo_chg_cur_det defination*/
#include <cust_charging.h>
/*lenovo-sw weiweij added for lenovo_chg_cur_det defination end*/
...
kal_int32 lenovo_read_charging_cur(void *data)
{    
#if defined(CONFIG_POWER_EXT)
    *(kal_int32*)(data) = 5001;
#else
    int ret = 0, buf[4], i, ret_value = 0, ret_temp = 0;
    int Channel=LENOVO_CHARGING_CUR_CHANNEL;
    int times = 2;
    ...
    while (i--)
    {
        ret_value = IMM_GetOneChannelValue(Channel, buf, &ret_temp);
        ret += ret_temp;
        bm_print(BM_LOG_FULL, "[get_tbat_volt] ret_temp=%d\n",ret_temp);
    }
    ...
    ret = ret*1500/4096 ;
    ret = ret/times;
    bm_print(BM_LOG_CRTI, "[%s]  output mV = %d\n",__func__, ret);

	ret = ret * 680 / LENOVO_CHARGING_CUR_R;
    bm_print(BM_LOG_CRTI, "[%s] chg cur = %d\n",__func__, ret);
