...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
    {
        ...
        Afe_Block->u4WriteIdx  = Afe_Block->u4DMAReadIdx%Afe_Block->u4BufferSize;
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SET_HEADPHONE_ON:
            ...
            mt_set_gpio_dir(GPIO_HPPA_EN_PIN,GPIO_DIR_OUT); // output
            mt_set_gpio_out(GPIO_HPPA_EN_PIN,GPIO_OUT_ONE); // high
            msleep(15);
            #endif
            /* lenovo-sw zhouwl, 2014-01-08, use headphone pa for handset */
            #if defined(LENOVO_USE_HPTPA6136_CODEC) //stella
            mt_set_gpio_dir(GPIO_HPTPA6136_HI_Z_PIN, GPIO_DIR_OUT); //Hi_Z
            mt_set_gpio_out(GPIO_HPTPA6136_HI_Z_PIN, GPIO_OUT_ZERO);
        ...
        case SET_HEADPHONE_OFF:
            ...
            #if defined(LENOVO_USE_HP_CODEC) //sofina
            mt_set_gpio_dir(GPIO_HPPA_EN_PIN,GPIO_DIR_OUT);  // output
            mt_set_gpio_out(GPIO_HPPA_EN_PIN,GPIO_OUT_ZERO); // low
            msleep(15);
            #endif
            /* lenovo-sw zhouwl, 2014-01-08, use headphone pa for handset */
            #if defined(LENOVO_USE_HPTPA6136_CODEC) //stella
            ...
            mt_set_gpio_out(GPIO_HPTPA6136_GAIN_PIN, GPIO_OUT_ZERO);
            mt_set_gpio_dir(GPIO_HPTPA6136_SDB_PIN, GPIO_DIR_OUT); //SDB
            mt_set_gpio_out(GPIO_HPTPA6136_SDB_PIN, GPIO_OUT_ZERO);
            #endif
    ...
    while (count)
    {
        ...
        if (count != 0)
        {
            ...
            if ((ret <= 0) && (t2  > DL1_Interrupt_Interval * 1000000))
            {
                ...
                PRINTK_AUDDRV("AudDrv write t2 = %llu t1 = %llu t2-t1 = %llu WriteArrayIndex = %d\n", t2, t1, t2 - t1, WriteArrayIndex);
                PRINTK_AUDDRV("AudDrv write DataRemained:0x%x R:0x%x W:0x%x BufSize:0x%x",Afe_Block->u4DataRemained,Afe_Block->u4DMAReadIdx,Afe_Block->u4WriteIdx,Afe_Block->u4BufferSize);
...
ssize_t AudDrv_MEMIF_Read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        if (DMA_Read_Ptr + read_size < Afe_Block->u4BufferSize)
        {
            ...
            {
                PRINTK_AUDDRV("AudDrv_MEMIF_Read E1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x pucVirtBufAddr : %x \r\n",
