...
//lenovo.sw2 houdz 20140214 add:support  "Multi Gamma" and "PQ"function begin 
#ifdef LENOVO_LCM_MULTI_GAMMA_PQ 
static volatile int color_disable_bls = 0;
#endif
//lenovo.sw2 houdz 201340214 add:support  "Multi Gamma" and "PQ"function end
...
//lenovo.sw2 houdz 20140214 add:support  "Multi Gamma" and "PQ"function begin 
#ifdef LENOVO_LCM_MULTI_GAMMA_PQ 
static void bls_gamma_update_func(void* param)
{
    unsigned long CurVal;
    int i;
    const DISPLAY_GAMMA_T *gamma_index = (const DISPLAY_GAMMA_T *)param;

    #define GAMMA_ENTRY(r, g, b) \
        ( ((r & 0x3ff) << 20) | ((g & 0x3ff) << 10) | (b & 0x3ff) )
    ...
    for (i = 0; i < 256 ; i++)
    {
        CurVal = GAMMA_ENTRY(gamma_index->entry[0][i], gamma_index->entry[1][i], gamma_index->entry[2][i]);
        DISP_REG_SET(DISP_REG_BLS_GAMMA_LUT(i), CurVal);
        //DISP_DBG("[%d] GAMMA LUT = 0x%x, (0x%x)\n", i, DISP_REG_GET(DISP_REG_BLS_GAMMA_LUT(i)), CurVal);
    }
    ...
    CurVal = GAMMA_ENTRY(gamma_index->entry[0][256], gamma_index->entry[1][256], gamma_index->entry[2][256]);
    DISP_REG_SET(DISP_REG_BLS_GAMMA_BOUNDARY, CurVal);
        
    DISP_REG_SET(DISP_REG_BLS_LUT_UPDATE, 0);

    // enable BLS
    DISP_REG_SET(DISP_REG_BLS_EN, (DISP_REG_GET(DISP_REG_BLS_EN) | 0x1));
    
    DISP_MSG("bls_gamma_update_func: gamma updated\n");
...
{
    return (((1 << DISP_MODULE_COLOR) & u4UpdateFlag) || color_disable_bls) ? 1 : 0;
}
...
#endif
//lenovo.sw2 houdz 20140214 add:support  "Multi Gamma" and "PQ"function end
...
//            disp_set_needupdate(DISP_MODULE_BLS , 0);
		...
		//lenovo.sw2 houdz 20140214 add:support  "Multi Gamma" and "PQ"function begin
	  	...
	  	case DISP_IOCTL_SET_GAMMALUT:
            ...
            DSI_Config_DDPModule(bls_gamma_update_func, (void*)gamma_index);
...
//lenovo.sw2 houdz 20140214 add:support  "Multi Gamma" and "PQ"function end
        ...
        case DISP_IOCTL_SET_TPLAY_HANDLE:
            ...
            {
                DISP_ERR("DISP_IOCTL_SET_TPLAY_HANDLE, copy_from_user failed\n");
                return -EFAULT;
            }
            set_tplay_handle_addr_request();
            write_tplay_handle(value);
            break;
#endif
        default :
...
// secure video path implementation: a physical address is allocated to place a handle for decryption buffer.
    if (0 != init_tplay_handle()) // non-zero value for valid VA
    {
        ;
    }
#endif
