...
static bool dsi_log_on = true;
static bool glitch_log_on = false;
static bool force_transfer = false;
extern BOOL is_early_suspended;
static bool config_gamma_enable = false;
void (*callback_ddp)(void *param);
void *param_ddp;
typedef struct
{
	DSI_REGS regBackup;
...
//		DBG_OnTeDelayDone();
		...
		if(config_gamma_enable){
				DSI_SetMode(lcm_params->dsi.mode);
        
        callback_ddp(param_ddp);
        
				DSI_Start();
				config_gamma_enable = false;
...
UINT32 DSI_dcs_read_lcm_reg_v2(UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
    ...
    if (buffer == NULL || buffer_size == 0)
        return 0;
#ifndef BUILD_LK//lenovo jixu add
	if((lcm_params!=NULL)&&(lcm_params->dsi.hs_read)){
		DSI_clk_HS_mode(1);
		DSI_EnableClk();
	}
#endif
...
void DSI_Config_DDPModule(void (*callback)(void *), void *param)
{
	if (down_interruptible(&sem_early_suspend)) {
		printk("[DSI_ConfigBLSGamma] can't get semaphore()\n");
		return;
	}
	...
	if(lcm_params->dsi.mode == CMD_MODE){
		...
		long ret;
		static const long WAIT_TIMEOUT = 2 * HZ;    // 2 sec
		callback_ddp = callback;
		param_ddp = param;
		config_gamma_enable = true;
		DSI_SetMode(0);//stop DSI video mode
        
		ret = wait_event_interruptible_timeout(_dsi_wait_vm_done_queue,
													 !config_gamma_enable,
													 WAIT_TIMEOUT);
		if (0 == ret) {
			xlog_printk(ANDROID_LOG_WARN, "DSI_ConfigBLSGamma", " Wait for DSI video mode done timeout!!!\n");
