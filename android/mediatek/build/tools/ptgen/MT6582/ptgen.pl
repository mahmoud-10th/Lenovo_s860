...
#[heqi1] if exist row product, re-direct xls to vendor/{project}/*.xls
if (-e "$ENV{'LENOVO_PRODUCT_ROOT'}/$ENV{'MTK_PROJECT'}/partition_table_$ENV{'MTK_PROJECT'}.xls")
{
    $PART_TABLE_FILENAME = "$ENV{'LENOVO_PRODUCT_ROOT'}/$ENV{'MTK_PROJECT'}/partition_table_$ENV{'MTK_PROJECT'}.xls";
}
#[heqi1] end

#xiazs1, lenovo.sw :add for partiton override
if (-e "$LOCAL_PATH/partition_table_$ENV{'MTK_PROJECT'}.xls")
{
    print "xzs current dir $ENV{'PWD'} \n";
    print "xzs name $LOCAL_PATH/partition_table_$ENV{'MTK_PROJECT'}.xls\n";
    print "xzs, use project specific partition talbe\n";
    $PART_TABLE_FILENAME   = "mediatek/build/tools/ptgen/$PLATFORM/partition_table_$ENV{'MTK_PROJECT'}.xls";
}
#xiazs1, lenovo.sw : end add for partiton override
...
#lenovo-sw jixj 2013.8.8 add begin
my $bat_pl = "mediatek/build/tools/batgen/batgen.pl";
system("perl $bat_pl") == 0 or die "batgen gen return error\n";
#lenovo-sw jixj 2013.8.8 add end
PrintDependModule($0);
print "*******************Arguments*********************\n" ;
...
sub GenYAMLScatFile(){
	...
	my %fileHash=(
		...
		#lenovo-sw jixj 2014.3.17 add for fatimage
		FAT=> "fat_$PROJECT.img",
		PRELOAD=>"preload.img"  #lulu add for IC 2014.03.27
	);
