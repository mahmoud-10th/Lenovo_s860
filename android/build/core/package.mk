...
#
  ifneq ($(RESOURCE_OVERLAY_SUPPORT),)
    ...
    ifneq ($(strip $(overlay_manifest_file)),)
      LOCAL_FULL_MANIFEST_FILE := $(word 1,$(overlay_manifest_file))
    endif
...
#
# MediaTek resolution DFO overlay
#
ifeq ($(strip $(MTK_DFO_RESOLUTION_SUPPORT)),yes)
  MTK_RESOURCE_DFO_OVERLAYS_TEMP := $(strip $(call mtk-resource-overlays-rule,$(LOCAL_PATH),$(LOCAL_RESOURCE_DIR),$(MTK_RESOURCE_DFO_OVERLAYS_CHOICE),$(MTK_RESOURCE_DFO_OVERLAYS_FOLDER),$(MTK_RESOURCE_DFO_OVERLAYS_MAPPING),$(MTK_RESOURCE_DFO_OVERLAYS_OUTPUT)))
  ifneq ($(strip $(LOCAL_GENERATED_RESOURCES)),)
    $(info LOCAL_PATH = $(LOCAL_PATH))
    $(info LOCAL_RESOURCE_DIR = $(LOCAL_RESOURCE_DIR))
    $(info LOCAL_GENERATED_RESOURCES = $(LOCAL_GENERATED_RESOURCES))
  endif
endif
...
all_resources := $(strip $(all_resources) $(LOCAL_GENERATED_RESOURCES))
...
#wangxuejin,begin,20111014
#add for mm, when mm all res will build again.mustbe add after R_file_stamp.
ifneq (,$(ONE_SHOT_MAKEFILE))
ifneq (,$(R_file_stamp))
$(warning wangxuejin, we always rebuild app res when mming.)
else
$(warning wangxuejin, mming R_file_stamp is null.)
endif

rebuild_res_asset_list:=wangxuejin
.PHONY:$(rebuild_res_asset_list)
$(R_file_stamp):$(rebuild_res_asset_list)
endif
#wangxuejin,end,20111014
...
ifneq ($(filter user userdebug, $(TARGET_BUILD_VARIANT)),)
    ...
    ifeq ($(MTK_PROGUARD_SHRINKING),yes)
...
LOCAL_PROGUARD_FLAGS += $(LOCAL_PROGUARD_FLAGS) -printusage $(package_expected_intermediates_COMMON)/shrink.txt -printseeds $(package_expected_intermediates_COMMON)/seed.txt
...
# begin, lenovo xucw 20130620 ltm-1 for support IdeaUI
$(call intermediates-dir-for,APPS,lenovo-res,,COMMON)/package-export.apk:$(call intermediates-dir-for,APPS,framework-res,,COMMON)/src/R.stamp
# end, lenovo xucw 20130620 ltm-1 for support IdeaUI
...
# begin, lenovo xucw 20130620 ltm-1 for support IdeaUI
ifneq ($(LOCAL_NO_LENOVO_RES),true)
framework_res_package_export += \
    $(call intermediates-dir-for,APPS,lenovo-res,,COMMON)/package-export.apk
endif
# end, lenovo xucw 20130620 ltm-1 for support IdeaUI
