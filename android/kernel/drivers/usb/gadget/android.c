...
/*Lenovo-sw yexh1 modified 2014-3-27, add for support mirrorlink NCM func support*/
#if defined(LENOVO_MIRRORLINK_NCM)
#include "f_ncm.c"
#include <linux/netlink.h>
#define MIRRORLINK_REQUEST_START    0xF0
#define NETLINK_MIRRORLINK_REQ       31
#endif
/*Lenovo-sw yexh1 modified 2014-3-27 end*/
...
#define NLUN_STORAGE 1
     ...
     /*Lenovo-sw yexh1 add 2013-05-15,add for CD-Rom */ 
    #ifdef MTK_BICR_SUPPORT 
	//config->fsg.nluns = NLUN_STORAGE;
	config->fsg.nluns = NLUN_STORAGE+1;
	...
	if (IS_ERR(common)) {
     ...
     /*Lenovo-sw yexh1 add 2013-06-26,add for CD-Rom */ 
#ifdef MTK_BICR_SUPPORT
...
#endif 
  /*Lenovo-sw yexh1 add 2013-06-26,add for CD-Rom, end */ 
	/*
...
struct ncm_function_config {
	u8 ethaddr[ETH_ALEN];
	struct sock *eventSocket;
	struct ncm_mirrorlink_info reqInfo;
};
...
static int ncm_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	...
	pr_err("%s: malloc successd\n", __func__);
	ncm = (struct ncm_function_config *)(f->config);
	ncm->eventSocket = netlink_kernel_create(&init_net, NETLINK_MIRRORLINK_REQ, 0,
												NULL, NULL, THIS_MODULE);
	if (NULL == ncm->eventSocket) {
...
{
	struct ncm_function_config *ncm = (struct ncm_function_config *)(f->config);

	pr_err("%s: start cleanup\n", __func__);
	if (NULL != ncm->eventSocket) {
		netlink_kernel_release(ncm->eventSocket);
	}
	kfree(f->config);
	f->config = NULL;
}
					...
					struct usb_configuration *c)
{
	...
	pr_err("%s: start ncm_bind_config\n", __func__);
	ret = ncm_bind_config(c, ncm->ethaddr);
	pr_err("%s:bind result:%d\n", __func__, ret);
	return ret;
}
...
static int ncm_send_netlink_info(struct sock *ssk, void *info, int len)
{
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
	int size;
	...
	ret = netlink_broadcast(ssk, skb, 0, 1, GFP_ATOMIC);
	pr_err("%s: broadcast result:%d\n", __func__, ret);
	return ret;
nlmsg_failure:
	if (NULL != skb) {
		kfree_skb(skb);
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	struct ncm_function_config *ncm = (struct ncm_function_config *)(ncm_function.config);
	struct ncm_mirrorlink_info *reqInfo = &(ncm->reqInfo);

	/* mirrorlink request */
	if ((USB_DIR_OUT | USB_TYPE_VENDOR) == b_requestType
		&& MIRRORLINK_REQUEST_START == b_request) {
		value = 0;
		reqInfo->major = w_value & 0xff;
		reqInfo->minor = w_value >> 8;
		reqInfo->vendorID = w_index;
		pr_err("%s: reqInfo:%d, %d, 0x%x\n", __func__, reqInfo->major, reqInfo->minor, reqInfo->vendorID);
		if (NULL != ncm->eventSocket) {
			ret = ncm_send_netlink_info(ncm->eventSocket, reqInfo, sizeof(*reqInfo));
			if (ret < 0) {
				pr_err("%s: ncm send mirrorlink request info failed(%d)\n", __func__, ret);
			}
		}
	}
