...
#include "../../bionic/libc/kernel/common/linux/mmc/sd_misc.h"
#endif
            ...
            while (fgets(buf, sizeof(buf), fp_info)) {
                ...
                if (sscanf(buf, "%s %s %s %d %s", p_name, p_size, p_addr, &p_type, p_actname) == 5) {
                    ...
                    if (!strcmp(p_name, "preloader")) {
                        ...
                        has_custom = 1;
                    } else if (!strcmp(p_name, "preload")) {
                        snprintf(dev[PRELOAD_INDEX], sizeof(dev[PRELOAD_INDEX]), "%s", p_actname);
                    }
                }
            }
        }
            ...
            if (fstab_type != UNKNOWN_TYPE) {
                if (phone_type != fstab_type) {
                    ...
                    if (phone_type == EMMC_TYPE) {
                        ...
                        //preload
                        device_volumes[8].mount_point = strdup("/preload");
                        device_volumes[8].fs_type = strdup("ext4");
                        device_volumes[8].device = strdup(dev[PRELOAD_INDEX]);
                        device_volumes[8].device2 = NULL;
                        device_volumes[8].length = 0;

                        if (has_custom) {
                            //custom
                            device_volumes[9].mount_point = strdup("/custom");
                            device_volumes[9].fs_type = strdup("ext4");
                            device_volumes[9].device = strdup(dev[CUSTOM_INDEX]);
                        ...
                        if (has_custom) {
                            //sdcard2
                            if (has_fat) {
                                device_volumes[10].mount_point = strdup("/sdcard2");
                                device_volumes[10].fs_type = strdup("vfat");
#if defined(MTK_LCA_ROM_OPTIMIZE)
                                device_volumes[10].device = strdup("/dev/block/mmcblk1p1");
                                device_volumes[10].device2 = strdup("/dev/block/mmcblk1");
#else
#if defined(MTK_2SDCARD_SWAP)
                                device_volumes[10].device = strdup(dev[FAT_INDEX]);
                                device_volumes[10].device2 = NULL;
#else
                                device_volumes[10].device = strdup("/dev/block/mmcblk1p1");
                                device_volumes[10].device2 = strdup("/dev/block/mmcblk1");
#endif
#endif
                                device_volumes[10].length = 0;
                            } else {
                                //no 2nd SD
                                num_volumes--;
                            } // has_fat
                        ...
                        #if defined(UBIFS_SUPPORT)
                        device_volumes[9].device = strdup(dev[PRELOAD_INDEX]);
                        #else
                        device_volumes[9].device = strdup("preload");
                        #endif
        ...
        if (mount_point && fs_type && device) {
            ...
            if (phone_type == EMMC_TYPE) {
                ...
                if (!strcmp(mount_point, "/cache") && strcmp(device, dev[CACHE_INDEX])) {
                    ...
                    device_volumes[num_volumes].device2 = device2 ? strdup(device2) : NULL;
                } else if (!strcmp(mount_point, "/preload") && strcmp(device, dev[PRELOAD_INDEX])) {
                    device_volumes[num_volumes].device = strdup(dev[PRELOAD_INDEX]);
                    device_volumes[num_volumes].device2 = device2 ? strdup(device2) : NULL;
#if defined(MTK_LCA_ROM_OPTIMIZE)
