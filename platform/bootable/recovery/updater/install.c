                ...
                if (sscanf(buf, "%s %s %s %d %s", p_name, p_size, p_addr, &p_type, p_actname) == 5) {
                    ...
                    if (!strcmp(p_name, "preloader")) {
                        ...
                        if (phone_type == EMMC_TYPE) {
                            ...
                            snprintf(dev[PRELOAD_INDEX], sizeof(dev[PRELOAD_INDEX]), "preload");
                        }
                    }
                }
            }
...
#else
    if (!strcmp(mount_point, "/system") || !strcmp(mount_point, "/data") || !strcmp(mount_point, "/cache") || !strcmp(mount_point, "/custom") || !strcmp(mount_point, "/preload")) {
        ...
        if (!strcmp(mount_point, "/system")) {
            ...
            fprintf(stderr, "mount %s %s\n", mount_point, location);
        } else if (!strcmp(mount_point, "/preload")) {
            free(location);
            location = strdup(dev[PRELOAD_INDEX]);
            fprintf(stderr, "mount %s %s\n", mount_point, location);
        }
...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (!strcmp(mount_point, "/system") || !strcmp(mount_point, "/data") || !strcmp(mount_point, "/cache") || !strcmp(mount_point, "/custom") || !strcmp(mount_point, "/preload")) {
        ...
        if (!strcmp(mount_point, "/system")) {
            ...
            free(location);
            location = strdup(dev[PRELOAD_INDEX]);
            fprintf(stderr, "format /preload %s\n", location);
        }
  ...
  /* lenovo-sw :  , Remove content of /data partition excluding data/media files */
            ...
            {
                 printf("format content of /data partition excluding data/media files\n");
                 //MountExt4("ext4", "EMMC", "/data");   //"/data" has been mounted before  lenovo_ota_fix_patch_location();
                 //delete_data();
