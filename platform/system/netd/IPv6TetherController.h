...
private:
    ...
    int ifc_enable_allmc(const char *name);
    int ifc_disable_allmc(const char *name);
    int ifc_set_flags_build(const char *name, unsigned set, unsigned clr);
    void ifc_init_ifr_build(const char *name, struct ifreq *ifr);
};
